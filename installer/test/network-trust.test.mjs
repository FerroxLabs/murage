...
const MACOS_TAILNET = probeOf({
  ...
  utun4: [v4("100.64.0.10"), v6("fd7a:115c:a1e0::4d3b:796d")],
});
...
test("finds the tailnet addresses on a Linux node", () => {
  ...
  assert.deepEqual([...found].sort(), ["100.64.0.11", "fd7a:115c:a1e0::1234:5678"]);
  assert.equal(tailnetIpv4({ interfaces: LINUX_TAILNET }), "100.64.0.11");
});
...
test("finds the tailnet on macOS, where the device is a bare utun", () => {
  ...
  assert.deepEqual([...found].sort(), ["100.64.0.10", "fd7a:115c:a1e0::4d3b:796d"]);
});
...
test("arrivedOverTailnet compares the LOCAL address, not the peer", () => {
  ...
  assert.equal(arrivedOverTailnet("100.64.0.11", opts), true);
...
test("the CGNAT operator override forces the rule both ways", () => {
  ...
  assert.equal(
    arrivedOverTailnet("100.64.0.11", { ...base, env: { MURAGE_TAILSCALE_CGNAT_OPERATOR: "0" } }),
    false
  );
});
...
test("classifyClientTrust: a CGNAT peer is operator only when it landed on the tailnet", () => {
  const opts = { interfaces: LINUX_TAILNET, env: {} };
  assert.equal(classifyClientTrust("100.109.207.54", "100.64.0.11", opts), "operator");
