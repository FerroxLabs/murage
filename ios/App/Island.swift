                ...
                if let shown {
                    VStack(spacing: 10) {
                        ...
                        Button { open(shown.chat) } label: {
                            ChatAvatarView(chat: shown.chat, size: 120, state: EmberState.forChat(shown.chat, in: session.state), animated: attentionLive, comets: attentionLive)
                        }
                        .buttonStyle(.plain)
                        ...
                        VStack(spacing: 4) {
                            Label("\(shown.chat.name) needs you", systemImage: "hand.raised.fill")
                                ...
                                .foregroundStyle(EmberPalette.color(shown.chat.color))
                                ...
                                if card.skillRequest != nil {
                                    Button {
                                        ...
                                        Label("Open chat to review", systemImage: "doc.text.magnifyingglass")
                                            ...
                                            .frame(height: 40)
                                            .background(Capsule().fill(EmberPalette.color(shown.chat.color)))
                                    }
                                    .buttonStyle(.plain)
                                } else {
                                    HStack(spacing: 8) {
                                        ForEach(card.options, id: \.self) { option in
                                            Button {
                                                ...
                                                Text(option)
                                                    ...
                                                    .frame(height: 40)
                                                    .background(
                                                        Capsule().fill(CardStyle.isRefusal(option) ? Color.white.opacity(0.16) : EmberPalette.color(shown.chat.color))
                                                    )
                                            }
                                            .buttonStyle(.plain)
                                            .disabled(answering)
