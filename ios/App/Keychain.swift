...
enum Keychain {
    static func save(_ token: String, for connectionId: String) throws {
        try MurageSharedKeychain.save(token, for: connectionId)
    }
    ...
    static func token(for connectionId: String) throws -> String? {
        try MurageSharedKeychain.tokenMigratingLegacyItem(for: connectionId)
    }

    @discardableResult
    static func remove(_ connectionId: String) -> Bool {
        MurageSharedKeychain.removeIncludingLegacyItem(connectionId)
    }
}

typealias KeychainError = MurageSharedKeychainError
