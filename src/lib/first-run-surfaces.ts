...
import { CHIEF_CONFIRMATIONS } from "../../shared/first-run-chief.ts";
import { SETUP_CARD_COPY } from "../../server/setup-conversation.ts";

import { FIRST_RUN_PHASES } from "./first-run.ts";
import { morningOffer } from "./first-run-flow.ts";
import {
...
export const FIRST_RUN_SURFACES = {
  ...
  // close.
  FIRST_RUN_PHASES,
  // THE FOURTH SURFACE, AND IT IS NOT IN THIS DIRECTORY. The Chief's own
  // confirmations are posted by the SERVER, so they cannot live in the
  // renderer's copy module, and while they sat as a literal inside
  // server/index.ts no rule on the gate reached them at all. That is how the
  // flow came to say "That is saved, and locked away on this computer" about
  // a key nothing had ever tried. They are shared/first-run-chief.ts now, and
  // the register test's area was widened to reach them, so a fifth sentence
  // added there is gated the moment it exists.
  CHIEF_CONFIRMATIONS,
} as const;
...
export const FIRST_RUN_SURFACE_MODULES: Readonly<Record<string, string>> = {
  ...
  "first-run-chief.ts": "surface: CHIEF_CONFIRMATIONS",
};
...
export function firstRunAssembledStrings(): ReadableString[] {
  return [
    ...
    // sentence that admits the immediate run is one of them. Walked
    // assembled, because the three sentence ceiling is about what lands on
    // the screen rather than about how it was stored.
