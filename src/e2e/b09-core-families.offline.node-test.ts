...
import {
  ...
  practiceSet, tableRow, toolsOnPath, evaluateJsFunction, workspaceLeakTexts,
...
import { installedPlaybookInstructions } from "../../server/installed-playbooks.ts";

test("Excel guidance reaches ordinary expense and CSV requests without matching unrelated words", () => {
  const book = JSON.parse(readFileSync(new URL("../../bot-library/builtins/excel-creator.json", import.meta.url), "utf8")).package.playbooks[0];
  const original = B09_CASES.find(item => item.id === "excel-creator/summary")!.turns[0]!;
  for (const text of [original, "Save this table as CSV.", "Check this expense report.", "Build a workbook from these rows."]) {
    assert.ok(installedPlaybookInstructions(text, [book]).includes("40 + 25 + 10 = 75"));
  }
  for (const text of ["Write a travel story.", "Explain an excellent result."]) {
    assert.equal(installedPlaybookInstructions(text, [book]), "");
  }
});
...
test("denied-source scan handles internal skill-directory links without hiding copied canaries", () => {
  const root = mkdtempSync(join(tmpdir(), "b09-leak-scan-"));
  try {
    mkdirSync(join(root, "skills"));
    writeFileSync(join(root, "private.md"), "CANARY");
    writeFileSync(join(root, "skills", "SKILL.md"), "safe skill");
    symlinkSync(join(root, "skills"), join(root, "skill-alias"), "dir");
    assert.deepEqual(workspaceLeakTexts(root, "private.md"), ["safe skill"]);
    writeFileSync(join(root, "skills", "copied.md"), "CANARY");
    assert.ok(workspaceLeakTexts(root, "private.md").includes("CANARY"));
    symlinkSync(join(root, "private.md"), join(root, "source-alias.md"));
...
test("CSV, Markdown table and practice-set readers extract the deterministic facts deliverable checks use", () => {
  ...
  assert.deepEqual(practiceSet("## Questions\n**Q1 (Free Recall):** Name the plant parts and their jobs.\n**Q2 (Application):** Correct the swapped jobs.\n## Answer Key\n**Q1:** Roots absorb water.\n**Q2:** Leaves use light."), { questions: 2, answerKeyAfterQuestions: true });
  assert.deepEqual(practiceSet("**Q1:** \n**Q2 (Application):**\n## Answer Key\n**Q1:** Water."), { questions: 0, answerKeyAfterQuestions: false });
  assert.deepEqual(practiceSet("## Answer Key\n**Q1:** Water.\n## Questions\n**Q1:** Name the plant parts."), { questions: 0, answerKeyAfterQuestions: false });
  assert.deepEqual(practiceSet("**Q1:** What do roots absorb?\n## Answer Key\nWater."), { questions: 1, answerKeyAfterQuestions: true });
