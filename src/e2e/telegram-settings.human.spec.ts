...
test('Telegram token, pairing, refresh and revoke remain deliberate with recoverable failures',async({page},testInfo)=>{
  ...
  let state={configured:false,enabled:false,paired:false,pending:false,uncertain:false,connecting:false};let pairs=0;let fail=false;
  ...
  await page.route('**/api/telegram/revoke',route=>{state={...state,enabled:false,paired:false,pending:false};return route.fulfill({json:{}});});
  ...
  state={...state,paired:true,pending:false};await page.getByRole('button',{name:'Refresh status'}).click();
  ...
  await page.screenshot({path:testInfo.outputPath('telegram-settings-mobile.png')});
});
