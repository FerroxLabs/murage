...
import {
  ...
  exportNativeLogs, fileSize, listFiles, liveInputGaps, nativeStops, publicDescriptor, prepareReadOnlyTaskWorkspace, readCalendarReceipt, readCredential, startIsolatedHarness, unexecutedStatus, writeReceipt,
...
const SCENARIOS: Record<B08Case["id"], (run: CaseRun) => Promise<void>> = {
  ...
  "cowork/missing-capability": async (run) => {
    ...
    const frozen = prepareReadOnlyTaskWorkspace({ repoRoot: ROOT, dataDir: need().harness.dataDir, home: need().harness.home, botId: bot.id, threadId });
    const { workspace } = frozen, before = listFiles(workspace);
    run.controlledState.proof = { workspace, mode: "0555 on all task directories after production metadata preparation", filesBefore: frozen.filesBefore, bundleId: frozen.bundleId, nativeMetadataReady: frozen.nativeMetadataReady, writesDenied: frozen.writesDenied };
    let turn: TurnRecord;
    try { turn = await runTurn(run, bot, threadId, run.item.turns[0]!); frozen.assertUnchanged(); }
    finally { frozen.restore(); }
