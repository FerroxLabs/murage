...
import { listFiles } from "./b08-template-behavior-fixture.ts";
...
export function practiceSet(text: string): { questions: number; answerKeyAfterQuestions: boolean } {
  ...
  const questions = (keyIndex >= 0 ? lines.slice(0, keyIndex) : lines).filter((line) => {
...
/** Directory aliases inside the workspace have their real files scanned already. */
export function workspaceLeakTexts(workspace: string, excludedFile: string): string[] {
  ...
  for (const file of listFiles(workspace)) {
    ...
    if (target === ".." || target.startsWith("../") || target.startsWith("..\\") || isAbsolute(target)) {
      throw new Error("Leak scan refuses a link outside the controlled workspace");
    }
    const stat = statSync(path);
    if (stat.isDirectory()) continue;
    if (!stat.isFile()) throw new Error("Leak scan encountered a non-file entry");
    if (stat.size < 2_000_000) texts.push(readFileSync(path, "utf8"));
  }
  return texts;
}
