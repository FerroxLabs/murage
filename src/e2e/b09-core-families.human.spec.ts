...
import { harnessPath, inspectDeliverable, inspectionSummary, toolsOnPath } from "./b09-b10-family-fixture.ts";
...
const HANDLERS: Record<B09Kind, (run: Run) => Promise<void>> = {
  ...
  async "denied-access"(run) {
    ...
    if (denied) {
      ...
      const texts = [runner.replyText(turn), ...listFiles(workspace).filter((f) => f !== denied.path && fileSize(join(workspace, f)) >= 0 && fileSize(join(workspace, f)) < 2_000_000).map((f) => readFileSync(join(workspace, f), "utf8"))];
