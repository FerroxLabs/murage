...
export function OffsiteDetails({r}:{r:RemoteController}){
 ...
 return <div className="space-y-3">
  <p className="text-[13px] text-ink-secondary">Keep a second encrypted copy in cloud storage you already have. It works with S3-compatible storage, such as Amazon S3, Backblaze B2, Cloudflare R2 or MinIO. Saving settings doesn't connect or upload anything.</p>
  {status?.supported&&<>
...
export function OffsiteRecover({r}:{r:RemoteController}){
 ...
 return <div className="space-y-3">
  <p className="text-[13px] text-ink-secondary">You can recover an off-site copy even if the original local backup is gone. Finding and downloading use your saved storage connection. Your current Murage data stays unchanged.</p>
