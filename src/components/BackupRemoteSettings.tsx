  ...
  {status?.supported&&<>
   ...
   {(!configured||editing)&&<div className="space-y-3">
    ...
    {kind==="s3"?<form className="space-y-3" onSubmit={event=>{event.preventDefault();if(locked||!payload)return;void run("save",async(api,expected)=>{
     ...
     <p className="text-[13px] text-ink-secondary">Use the endpoint, bucket, region and access keys from your storage provider. The bucket must already exist. Test connection creates the backup repository in it when there isn't one yet.</p>
   ...
   {configured&&!editing&&<div className="space-y-3">
    ...
    <p className="text-[13px] text-ink-secondary">Off-site password (file): {status.passwordSelected?"chosen":"not chosen"}. It locks the off-site copy and is separate from your recovery key and {sftp?"the SSH key":"storage access keys"}.</p>
    ...
    <div className="space-y-3 border-t border-hairline/40 pt-3">
     ...
     <p className="text-[13px] text-ink-secondary">When enabled, future locally verified backups are uploaded to this saved destination while Murage is open. Existing backups are not uploaded.{sftp?"":" Your provider may charge for transfers and storage."}</p>
