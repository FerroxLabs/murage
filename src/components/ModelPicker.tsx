...
import { pickerModels, orderedPickerModels, pickerKey, priceBand, contextLabel, engineFamilies, showNoLocalServerRow, localRowLabel, localRowNote, localToolsWarning, unavailableSelectionLabel, LOCAL_MODELS_GROUP, NO_LOCAL_SERVER_ROW } from "@/lib/provider-model-picker";
...
const noLocalServer=useMemo(()=>showNoLocalServerRow(engine,rows),[engine,rows]);
  ...
  // A thread picker whose trigger is disabled mid-turn stays closed: the event
  // must not bypass the guard the button itself enforces.
  useEffect(()=>{if(threadId&&bot.busy)return;const show=(event:Event)=>{const model=(event as CustomEvent<{model?:string}>).detail?.model;setEngineId(bot.modelSelection.instanceId);setQuery(typeof model==="string"?model:"");setLimit(30);setOpen(true);};window.addEventListener(OPEN_MODEL_PICKER_EVENT,show);return()=>window.removeEventListener(OPEN_MODEL_PICKER_EVENT,show);},[bot.modelSelection.instanceId,threadId,bot.busy]);
  let previousGroup="";
