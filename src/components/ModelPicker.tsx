...
import { pickerModels, pickerConnectionsToRefresh, orderedPickerModels, pickerKey, priceBand, contextLabel, engineFamilies, showNoLocalServerRow, localRowLabel, localRowNote, localToolsWarning, unavailableSelectionLabel, LOCAL_MODELS_GROUP, NO_LOCAL_SERVER_ROW } from "@/lib/provider-model-picker";
...
export function ModelPicker({bot,threadId,className,contained=false,label}:{bot:Bot;threadId?:string;className?:string;contained?:boolean;label?:ReactNode}){
  ...
  useLayoutEffect(()=>{
    ...
    const reposition=()=>{
      if(root.current&&menu.current)setMenuOffset(modelPickerViewportOffset(root.current.getBoundingClientRect().right,menu.current.getBoundingClientRect().width,window.innerWidth));
    };
    reposition();
