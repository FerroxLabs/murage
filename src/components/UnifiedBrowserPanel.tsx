...
type Status = { generation: number; held: boolean; connected: boolean; protectedDocument: boolean; url: string };
export function UnifiedBrowserPanel({ bot, size = "compact", onExpand }: { bot: Bot; size?: "compact" | "expanded"; onExpand?: () => void }) {
  ...
  const input = (event: Record<string, unknown>) => {
    ...
    if (!current.current?.held || pending) return;
  ...
  return <div className="flex min-h-0 flex-col gap-3 overflow-y-auto p-1">
    ...
    <div className="relative overflow-hidden rounded-xl border border-hairline"><BrowserLiveView frame={frame} generation={status?.generation ?? 0} held={!!status?.held && !pending} onInput={input} />{!frame && <div className="pointer-events-none absolute inset-0 flex items-center justify-center text-sm text-ink-secondary" role="status">{status?.connected ? "Waiting for the page…" : "Connecting to browser…"}</div>}</div>
    <div className="flex flex-wrap items-center justify-between gap-2"><span role="status" className="text-xs text-ink-secondary">{status?.connected ? "Live" : "Disconnected"} · {status?.held ? "Human control" : "Agent control"}</span><button disabled={!status || pending} onClick={() => void action(status?.held ? "release" : "take")} className="flex items-center gap-2 rounded-lg bg-accent px-3 py-2 text-sm text-accent-ink disabled:opacity-40 focus-visible:ring-2 focus-visible:ring-accent"><Hand size={14} />{pending ? "Please wait…" : status?.held ? "Return to agent" : "Take control"}</button></div>
