    ...
    <main aria-label="Set up your workspace" className="flex max-h-full w-full max-w-[620px] flex-col overflow-y-auto rounded-2xl border border-hairline/40 bg-panel p-8 max-md:p-5">
      {workspace === "checking" ? <p role="status" className="text-ink-secondary">Checking your workspace…</p> : workspace === "error" ? <>
        ...
        {step === 1 && <>
          ...
          {/* The chosen engine's setup panel sits UNDER ITS OWN ROW, not
              under the whole list. A machine with a dozen engines pushed it
              past the fold, so on a 1440x900 laptop the only control that
              could finish Fuigo's setup ("Open Flux Router in Models") was
              off screen with nothing saying it existed. */}
          <div className="mt-4 grid gap-2" aria-label="Choose an engine">{state.instances.filter(instance => instance.enabled !== false).map(instance => <Fragment key={instance.instanceId}><button aria-pressed={instanceId === instance.instanceId} className={"flex items-center gap-3 rounded-xl border p-3 text-left " + (instanceId === instance.instanceId ? "border-accent bg-accent/5 " : "border-hairline/50 bg-card ") + focus} onClick={() => { setInstanceId(instance.instanceId); setModel(""); }}><ProviderMark driverKind={instance.driverKind} size={20} /><span className="flex-1 text-sm text-ink">{instance.displayName}{instance.driverKind === "fuigoAgent" && <> <span className="ml-2 text-xs text-ink-secondary">Included</span></>}</span>{instanceId === instance.instanceId && <Check size={16} className="text-accent" />}</button>{selected && selected.instanceId === instance.instanceId && (!ready || !selected.models.options.length) && <EngineSetup instance={selected} intent={selected.access === "custom" ? "inject" : "cloud"} />}</Fragment>)}</div>
