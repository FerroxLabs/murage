...
1. **Result proof** - specific numbers, specific outcomes, specific timelines. Not "hundreds of clients" but "14 seed rounds in 36 months."
2. **Story proof** - one brief client story (50-100 words) that mirrors the reader's situation. Name the type of client, the starting problem, the result. No names required but no vagueness allowed ("a founder in e-commerce" is fine; "a client" is not).
3. **Authority proof** - the credential that earns the right to the claim. This is the only section where job titles, press mentions, and institutional affiliations appear. Keep it short and relevant. One credential that directly supports the promise beats five credentials that prove breadth.
...
4. **Proof points** - at least one number, one client story fragment, one credential
5. **The contrarian POV** - what does the seller do differently that explains their results?
...
1. **Section-by-section content brief** - what each section needs to accomplish, with the seller's raw inputs mapped to the framework
2. **Full page copy** - all six sections written and sequenced, in the seller's voice, with DR structure embedded
3. **CTA block** - the close section as a standalone block, ready to be styled separately
4. **Conversion checklist** - section-by-section pass/fail against the DR Arc criteria
...
- This skill pairs upstream with `content-bio` (Tier 3 bio seeds the discovery story) and downstream with `content-speaker-kit` (the About page proof section feeds the speaker one-sheet).
