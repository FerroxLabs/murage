---
name: voiceprint-voice-interview
description: "**Mode skill.** Default-enabled on the Voiceprint specialist. Runs in the Full path; skipped in the Lite and Refresh paths."
metadata:
  author: wayland
  version: "1.0.0"
  category: "voiceprint"
...
**Mode skill.** Default-enabled on the Voiceprint specialist. Runs in the Full path; skipped in the Lite and Refresh paths.

## When to use

Use after samples are gathered, before Compile. Goal: the meta-layer samples cannot show — who the user writes for, what they refuse to write, who they reject as a stylistic model, what they wish they did more of. Skip in the Lite path; a one-paragraph audience note replaces the interview.

Trigger phrases that should activate this mode:

- "Build my voice profile."
- "Let's do the full version."
- "I have samples — what's next?"
- "Run the 22 questions."

## Procedure

**1. Confirm samples are in hand.** Before question one, confirm 5–10 samples (and ideally 1–2 transcripts) are in the chat. If not, route back to gather. Interview-without-samples is the failure mode this method prevents.
...
- **One question at a time, always.** Batching feels efficient and destroys signal. Rapid-fire flattens the surfacing work.
- **Push back once, not twice.** First push edits; second push performs.
- **Capture verbatim.** Keep the user's exact words. The Compiler needs them.
- **Vague is data.** "I don't know who I write for" is a real answer. Note it; don't invent.
- **Self-contradiction is gold.** "I never hedge — but I always say 'I think' first." Compile will surface it.

## Anti-patterns

- **Summarizing as you go.** Premature pattern-matching anchors Compile to your interview-time guess, not the samples.
- **Accepting "engaging," "authentic," "conversational," or "professional but approachable" without a follow-up.** These words name nothing. Push back every time.
- **Leading the witness.** "Would you say you write like a Substack writer?" Ask open questions; tolerate silence.
- **Running the interview without samples.** You'll produce an aspirational profile that contradicts the actual writing.
- **Reordering the clusters.** Audience first (concrete), tone next (reflective), then refusals, style, influences, stance. Reordering breaks the warm-up.
