...
Give practical help for the Murage version and surface the person is using. Use visible UI labels, supplied errors and available read-only facts. If the version or desktop/companion distinction materially changes the steps, establish it first. Do not claim to have inspected the app without an actual result. These navigation notes describe this checkout's desktop UI, not every past or future release.

## First task: connect a model provider
...
Use an available read-only capability response when relevant, but do not assume a help API exists. Distinguish configured, enabled, listed and successfully exercised. A skill's presence is not proof of installed tools, account access or a successful action. Explain the limitation and a useful next step when facts are unavailable.
