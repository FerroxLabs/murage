...
> **Power-toolkit shortcuts:** for systematic scoring across a set of strategy parameters, prefer `strategy_sweep` over a hand-rolled loop — it caps combinations, supports resume-from-partial, and uses a short cooldown when only inputs change. Use `state_snapshot` to preserve the user's start state before scanning, and `watchlist_get` if the user said "scan my watchlist".
