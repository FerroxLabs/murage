...
if [ -n "${MURAGE_POSTINSTALL_TEST_ROOT:-}" ]; then
  ...
  case "$TEST_ROOT" in
    /tmp/*) ;;
    *) echo "Murage test install root must stay under /tmp" >&2; exit 1 ;;
  esac
...
if [ "$TEST_MODE" -eq 0 ]; then
  if command -v update-alternatives >/dev/null 2>&1; then
    update-alternatives --remove murage /opt/Murage/murage
  elif [ -L /usr/bin/murage ] && [ "$(readlink /usr/bin/murage)" = /opt/Murage/murage ]; then
    rm -- /usr/bin/murage
  fi
fi

profile=$APPARMOR_DIR/murage-browser
if [ ! -e "$profile" ] && [ ! -L "$profile" ]; then exit 0; fi
if [ -L "$APPARMOR_DIR" ] || [ -L "$profile" ] || [ ! -f "$profile" ]; then
  echo "Murage AppArmor profile is unsafe; refusing to remove it: $profile" >&2
  exit 1
fi
if [ "$TEST_MODE" -eq 0 ] && [ -x /usr/bin/ischroot ] && /usr/bin/ischroot; then
...
elif [ -r "$APPARMOR_PROFILES" ] && ! grep -q '^murage-browser ' "$APPARMOR_PROFILES"; then
  ...
  if ! "$APPARMOR_PARSER" -R "$profile"; then
    echo "Murage could not unload its browser AppArmor profile: $profile" >&2
    exit 1
  fi
elif [ -r "$APPARMOR_PROFILES" ]; then
  echo "Murage needs apparmor_parser to unload its browser profile: $profile" >&2
  exit 1
fi
rm -- "$profile"
