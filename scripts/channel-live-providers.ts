...
export async function handleFixtureReplySetup(harness: Harness, platform: Platform, chiefId: string, beforeRuns: Set<string>, instanceId: string, handled: Set<string>): Promise<void> {
  ...
  if (!run.threadId || run.status === "completed") return;
  ...
  const expectReplies = async (id: string, criterion: string, expectedTotal: number, expectedResponse?: string, beforeRuns?: Set<string>) => {
    ...
    await wait(criterion, async () => {
      if (descriptor && !buttonPhase && beforeRuns && state.chiefId)
        await handleFixtureReplySetup(harness!, platform, state.chiefId, beforeRuns, descriptor.instanceId, handledFixtureSetup);
