...
const runtime = process.env.MURAGE_CUA_RUNTIME || "docker";
const container = process.env.MURAGE_CUA_CONTAINER;
const socket = process.env.MURAGE_CUA_SOCKET || "/run/user/1000/murage-cua.sock";
if (!container || !/^[a-zA-Z0-9_.-]+$/.test(container)) {
  throw new Error("set MURAGE_CUA_CONTAINER to the explicitly created smoke container name");
}
...
const child = spawn(
  ...
  [
    ...
    container,
    "/usr/local/libexec/murage/cua-driver",
    "mcp",
    "--socket",
    socket,
  ],
...
try {
  await rpc("initialize", {
    ...
    capabilities: {},
    clientInfo: { name: "murage-container-smoke", version: "1" },
  });
