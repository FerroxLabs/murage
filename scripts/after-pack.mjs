 ...
 * smoke test and ship full-size photos to phones: refuse it here instead. */
const SHARP_LIBVIPS = {
...
export async function validatePackagedThumbnailRuntime(server, manifest, platform, arch) {
  const target = `${platform}-${arch}`;
  const entries = manifest.packages?.filter(entry => entry.name === "sharp");
  if (!Array.isArray(entries) || entries.length !== 1) throw new Error("Packaged thumbnail runtime (sharp) is missing from the memory runtime manifest");
  ...
  catch (error) { if (error?.code === "ENOENT") throw new Error(`Packaged thumbnail runtime is missing for ${target}: ${record(addon)}`); throw error; }
  ...
  if (names.length !== 1) throw new Error(`Packaged thumbnail runtime is missing libvips for ${target}: ${record(libDirectory)}`);
...
export async function validatePackagedMemoryRuntime(resources, platform, archValue, required = true) {
  ...
  const thumbnailBackend = await validatePackagedThumbnailRuntime(server, manifest, platform, arch);
...
export default async function afterPack(context) {
  ...
  if (context.electronPlatformName === "win32" && context.packager) {
    ...
    const backupDirectory = path.join(resources, "backup-tools", "x64");
    const backupHelper = path.join(backupDirectory, "murage-backup-age.exe");
    verifyWindowsBackupTools(backupDirectory);
    await requireRegularFile(backupHelper);
    const backupStat = await lstat(backupHelper);
    if (backupStat.nlink !== 1 || backupStat.size < 1 || backupStat.size > 64 * 1024 ** 2) throw new Error("Packaged Windows backup helper is unsafe");
    const executables = [
    ...
    verifyWindowsBackupTools(backupDirectory);
    await verifyWindowsBrowserSignatures([browser.engine, browser.chrome, recovery, backupHelper, ...(fuigoProbe ? [fuigoProbe.file] : [])], process.env.SystemRoot);
