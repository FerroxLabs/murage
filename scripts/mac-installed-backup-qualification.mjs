  ...
  function awaitState(fn,label){
   ...
   for(;;){
    ...
    try{var result=fn();if(result)return result;}
     ...
     // is opening or closing, or a child may become stale while Go To disappears.
     ...
     var closing=label==='picker-dismissed'&&error.error==='AX-read'&&error.attribute==='AXWindows'&&error.code===-25204;
     if(!opening&&!dismissing&&!closing)throw error;
