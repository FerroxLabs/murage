...
const hardDeath = process.env.MURAGE_SMOKE_HARD_DEATH === "1";
const bundled = process.env.MURAGE_SMOKE_BUNDLED_CUA === "1";
const sessionBlocked = process.env.MURAGE_SMOKE_LINUX_CUA_BLOCKED === "1";
const signalShutdown = process.env.MURAGE_SMOKE_SIGNAL_SHUTDOWN === "1";
...
const executable = path.resolve(
  process.env.MURAGE_SMOKE_EXECUTABLE ?? path.join(root, "release", "linux-unpacked", "murage"),
);
if (!existsSync(executable)) throw new Error(`[smoke-linux-package] missing executable: ${executable}`);

const sandbox = mkdtempSync(path.join(tmpdir(), "murage-linux-smoke-"));
...
mkdirSync(path.join(home, ".murage"), { recursive: true });
mkdirSync(xdgConfig, { recursive: true });
mkdirSync(xdgRuntime, { recursive: true, mode: 0o700 });
chmodSync(xdgRuntime, 0o700);
writeFileSync(
  path.join(home, ".murage", "config.json"),
  JSON.stringify({ instances: { ghost: { driver: "not-a-real-driver", displayName: "Ghost" } } }),
);
for (const appName of ["murage", "Murage"]) {
...
const desktopEnv = {
  ...
  MURAGE_COMPOSIO_BROKER_URL: `http://127.0.0.1:${brokerAddress.port}`,
  MURAGE_SMOKE_TEST: "1",
  MURAGE_SMOKE_CUA: hardDeath || bundled || sessionBlocked ? "0" : "1",
  MURAGE_SMOKE_BUNDLED_CUA: bundled ? "1" : "0",
};
if (hardDeath || signalShutdown) desktopEnv.MURAGE_SMOKE_KEEP_OPEN = "1";
...
try {
  ...
  if (health?.app !== "murage" || health.static !== true) {
    throw new Error(`unexpected embedded health response: ${JSON.stringify(health)}`);
  }
  if (!String(title).includes("Murage")) throw new Error(`unexpected renderer title: ${title}`);
  ...
  if (sessionBlocked) {
    ...
    const activeUserData = ["murage", "Murage"]
  ...
  if (hardDeath) {
    ...
    const restart = spawn(executable, wayland ? ["--ozone-platform=x11"] : [], {
      cwd: root,
      detached: true,
      env: { ...desktopEnv, MURAGE_SMOKE_KEEP_OPEN: "0" },
      stdio: ["ignore", "pipe", "pipe"],
    });
    try {
