...
export function validateBaseUrl(url: string): string {
  ...
  try {
    ...
    throw new Error(`Invalid Murage URL: '${url}'`);
  }
  if (parsed.protocol !== "http:" && parsed.protocol !== "https:") {
    throw new Error("Murage URL must use http:// or https://");
  }
  if (parsed.username || parsed.password) {
    throw new Error("Murage URL must not contain credentials; use MURAGE_TOKEN instead");
  }
  if ((parsed.pathname !== "/" && parsed.pathname !== "") || parsed.search || parsed.hash) {
    throw new Error("Murage URL must be an origin without a path, query, or fragment");
  }
...
const configuredUrl = process.env.MURAGE_URL ||
  (process.env.MURAGE_PORT ? `http://127.0.0.1:${process.env.MURAGE_PORT}` : undefined);

export const MURAGE_BASE_URL = validateBaseUrl(configuredUrl || "http://127.0.0.1:8799");
...
export function log(msg: string) {
  process.stderr.write(`[murage-mcp] ${msg}\n`);
}

export const DEFAULT_REQUEST_TIMEOUT_MS = 30_000;

function requestTimeoutMs(): number {
  const raw = Number(process.env.MURAGE_MCP_TIMEOUT_MS);
...
async function fetchJson(url: string, options: RequestInit = {}): Promise<any> {
  ...
  try {
    ...
    throw new Error(`Murage API returned a non-JSON response from ${url}`);
  }
}
...
export async function probeBaseUrls(candidates: string[]): Promise<string> {
  ...
  for (const unvalidated of candidates) {
    ...
    try {
      ...
      if (health?.app !== "murage") {
        failures.push(`${candidate} answered, but it was not Murage`);
        continue;
      }
      return candidate;
    } catch (error) {
  ...
  throw new Error(`Could not find a running Murage server. ${failures.join("; ")}`);
}

export async function resolveBaseUrl(): Promise<string> {
  if (discoveredBaseUrl) return discoveredBaseUrl;
  if (process.env.MURAGE_TOKEN?.trim() && !configuredUrl) {
    throw new Error("Set MURAGE_URL or MURAGE_PORT when using MURAGE_TOKEN so credentials are never sent during port discovery");
  }
...
export const TOOLS: McpToolDefinition[] = [
  {
    ...
    description: "Check whether the Murage server is reachable.",
    inputSchema: {
      type: "object",
      properties: {},
...
function idArg(args: Record<string, unknown>, key: string): string {
  ...
  if (!/^[\w-]+$/.test(value)) throw new ToolInputError(`${key} is not a valid Murage ID`);
  return value;
}
...
export async function handleToolCall(
  ...
  switch (name) {
    case "get_system_health": {
      ...
      if (res?.app !== "murage") throw new Error("The configured endpoint is not an Murage server");
      return {
        status: "connected",
        endpoint: discoveredBaseUrl ?? MURAGE_BASE_URL,
        app: "murage",
    ...
    case "create_bot": {
      ...
      if (!isRecord(created?.bot) || typeof created.bot.id !== "string") {
        throw new Error("Murage did not return the created bot");
      }
    ...
    case "update_bot_profile": {
      ...
      if (!isRecord(result?.bot)) {
        throw new Error("Murage did not return the updated bot");
      }
    ...
    case "create_channel": {
      ...
      if (!isRecord(created?.group) || typeof created.group.id !== "string") {
        throw new Error("Murage did not return the created channel");
      }
    ...
    case "update_channel": {
      ...
      if (!isRecord(result?.group)) {
        throw new Error("Murage did not return the updated channel");
      }
    ...
    case "create_task": {
      ...
      if (!isRecord(result?.task) || typeof result.task.threadId !== "string") {
        throw new Error("Murage did not return the created task");
      }
    ...
    case "rename_task": {
      ...
      if (!isRecord(result?.task)) {
        throw new Error("Murage did not return the renamed task");
      }
      return {
        success: true,
        targetType: args.target_type,
  ...
  try {
    ...
    if (method === "initialize") {
      ...
      return formatResponse(id, {
        ...
        instructions: "Use bounded read tools before mutating the Murage team. Approval grants, deletion, and computer lifecycle are intentionally unavailable.",
      });
    }
...
if (process.argv[1] && (process.argv[1].endsWith("mcp-server.ts") || process.argv[1].endsWith("mcp-server.js"))) {
  ...
  log("Murage MCP server running on stdio");
}
