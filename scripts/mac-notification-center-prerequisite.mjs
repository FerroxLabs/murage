...
export const NC_LABEL='com.apple.notificationcenterui';
export const NC_PLIST='/System/Library/LaunchAgents/'+NC_LABEL+'.plist';
...
export function notificationServiceState(run,uid){
 const domain='gui/'+uid,target=domain+'/'+NC_LABEL;
 ...
 const matches=[...disabled.stdout.matchAll(/^\s*"com\.apple\.notificationcenterui"\s*=>\s*(true|false)\s*$/gm)];requireValue(matches.length<=1,'NC_DISABLED_AMBIGUOUS');
 const value=matches.length?matches[0][1]==='true':null;
 const printed=run('/bin/launchctl',['print',target],{timeout:5000});
 if(printed.code!==0){requireValue(printed.code===113&&printed.stderr.includes('Could not find service "'+NC_LABEL+'"')&&printed.stderr.includes(String(uid)),'NC_JOB_UNREADABLE');return{disabled:value,loaded:false,pid:null};}
...
export function verifyNotificationServiceIdentity(run,{readFile=readFileSync,stat=lstatSync,realpath=realpathSync.native}={}){
 ...
 requireValue(plist.Label===NC_LABEL&&(plist.Program??plist.ProgramArguments?.[0])===NC_EXE,'NC_PLIST_IDENTITY');
 const bundle=run('/usr/bin/plutil',['-extract','CFBundleIdentifier','raw','-o','-',NC_APP+'/Contents/Info.plist'],{timeout:5000});requireValue(bundle.code===0&&bundle.stdout.trim()===NC_LABEL,'NC_BUNDLE_IDENTITY');
 ...
 return{label:NC_LABEL,plist:NC_PLIST,executable:NC_EXE,bundleId:NC_LABEL,plistSha256:createHash('sha256').update(readFile(NC_PLIST)).digest('hex'),appleSignatureVerified:true};
}
...
export async function prepareNotificationCenter({run,uid,persist,record,prior,guard=hostGuard,identity=verifyNotificationServiceIdentity,pause=ms=>new Promise(r=>setTimeout(r,ms))}){
 guard(run,uid);requireValue(!prior,'NC_PRIOR_ADMISSION_EXISTS');const id=identity(run),before=notificationServiceState(run,uid);
 ...
 let observed=notificationServiceState(run,uid);
 ...
 while(Date.now()<deadline){observed=notificationServiceState(run,uid);if(observed.loaded&&observed.pid&&observed.disabled!==true)break;await pause(250);}
 ...
 state.ready=true;state.readyPid=observed.pid;state.after=observed;persist(state);record('notification-service-ready',{pid:observed.pid,loaded:true,disabled:observed.disabled});return state;
}
export async function restoreNotificationCenter({run,uid,state,persist,record,guard=hostGuard,identity=verifyNotificationServiceIdentity,pause=ms=>new Promise(r=>setTimeout(r,ms))}){
 ...
 guard(run,uid);requireValue(state.version===1&&state.uid===uid&&state.identity.label===NC_LABEL&&state.identity.plist===NC_PLIST,'NC_RESTORE_IDENTITY');
 const id=identity(run);requireValue(id.plistSha256===state.identity.plistSha256,'NC_RESTORE_PLIST_CHANGED');
 const domain='gui/'+uid,target=domain+'/'+NC_LABEL,before=state.before;let now=notificationServiceState(run,uid);
 ...
 if(state.enableAttempted){requireValue(before.disabled===true,'NC_RESTORE_DISABLED_PREIMAGE');now=notificationServiceState(run,uid);if(now.disabled!==true){const disabled=run('/bin/launchctl',['disable',target],{timeout:5000});record('notification-service-command',{action:'restore-disable',code:disabled.code});requireValue(disabled.code===0,'NC_RESTORE_DISABLE_FAILED');}}
 const deadline=Date.now()+5000;for(;;){now=notificationServiceState(run,uid);if(now.loaded===before.loaded&&now.disabled===before.disabled)break;if(Date.now()>=deadline)throw Error('NC_RESTORE_READBACK');await pause(100);}
