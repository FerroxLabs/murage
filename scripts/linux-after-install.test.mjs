...
const browserPolicy = path.join(root, "build", "linux-murage-browser.apparmor");
...
function fixture() {
  ...
  fs.copyFileSync(browserPolicy, path.join(resources, "murage-browser.apparmor"));
  ...
  fs.writeFileSync(path.join(systemRoot, "apparmor-profiles"), "murage-browser (unconfined)\n");
...
describe("Linux DEB sandbox policy", () => {
  it("wires the exact Murage policy and hooks into Linux packaging", () => {
    const config = fs.readFileSync(path.join(root, "electron-builder.yml"), "utf8");
    expect(config).toContain("productName: Murage");
    expect(config).toContain("from: build/linux-murage-browser.apparmor");
    expect(config).toContain("to: murage-browser.apparmor");
    expect(config).toContain("afterInstall: build/linux-after-install.sh");
    expect(config).toContain("afterRemove: build/linux-after-remove.sh");
    expect(fs.readFileSync(hook, "utf8")).toContain("APP_ROOT=/opt/Murage");
  });
  ...
  it("allows user namespaces only for the exact installed browser executable", () => {
    ...
    expect(policy).toContain("profile murage-browser /opt/Murage/resources/browser-engine/chrome/chrome-headless-shell-linux64/chrome-headless-shell flags=(unconfined)");
...
describe.skipIf(process.platform !== "linux")("Linux DEB upgrade hook", () => {
  it("repairs legacy directory and executable modes idempotently", () => {
    ...
    for (let pass = 0; pass < 2; pass += 1) {
      ...
      expect(fs.readFileSync(path.join(apparmorDir, "murage-browser"), "utf8")).toBe(fs.readFileSync(browserPolicy, "utf8"));
      expect(result.stdout).toContain(`profile operation: -r ${apparmorDir}/murage-browser`);
    }
  });
  ...
  it("refuses an AppArmor destination symlink without overwriting its target", () => {
    ...
    fs.symlinkSync(external, path.join(apparmorDir, "murage-browser"));
    const result = runHook(appRoot);
    expect(result.status).not.toBe(0);
  ...
  it("keeps the profile on upgrade and removes only its own policy on uninstall", () => {
    const { appRoot, apparmorDir } = fixture();
    expect(runHook(appRoot).status).toBe(0);
    const profile = path.join(apparmorDir, "murage-browser");
  ...
  it("removes an already unloaded profile without asking the kernel again", () => {
    ...
    expect(result.stdout).not.toContain("profile operation");
    expect(fs.existsSync(path.join(apparmorDir, "murage-browser"))).toBe(false);
  });
  ...
  it("keeps the exact policy available for repair if the kernel refuses to unload it", () => {
    ...
    expect(fs.existsSync(path.join(apparmorDir, "murage-browser"))).toBe(true);
  });
  ...
  it("resolves the test root before applying the temporary-directory boundary", () => {
    ...
    // Preserve the traversal in the input even when TMPDIR is a nested fixture.
    const escaped = temporaryRoot + path.sep + path.relative(temporaryRoot, root);
