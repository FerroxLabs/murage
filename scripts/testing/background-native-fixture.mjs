...
try{
  ...
  lifecycle=createBackgroundLifecycle({platform:process.platform,window:()=>win,loadPreferences:()=>({}),savePreferences:()=>{},
    ...
    createTray:open=>{tray=new Tray(nativeImage.createFromPath(join(root,"electron/resources/app-icon.png")).resize({width:18,height:18}));tray.on("click",open);return tray;},
    setTrayMenu:(_tray,items)=>{menu=items;tray.setContextMenu(Menu.buildFromTemplate(items));},probeTray:()=>{const rect=tray.getBounds();return rect.width>0&&rect.height>0;},
    ...
    onChange:state=>{if(!win.isDestroyed())win.webContents.send("startup-background:changed",state);},onError:error=>{throw error;},
  });
  ...
  await lifecycle.start();assert.equal(lifecycle.status().trayAvailable,true);
...
finally{lifecycle?.dispose();if(win&&!win.isDestroyed())win.destroy();app.quit();}
