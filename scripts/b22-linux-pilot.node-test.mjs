...
test("a complete manifest yields bounded ordered operations without secret values",()=>{
  ...
  assert.match(text,/--no-install-tailscale/);assert.match(text,/no public share on this node/);
...
test("runtime paths are persisted through the generated command before service start",()=>{
  // Real private env-file helpers and shell quoting, with fake SSH/sudo only.
  const root=realpathSync(mkdtempSync(join(tmpdir(),"murage-b22-runtime-")));
  try{
    const m=valid(),plan=pilotPlan(m),release=`${m.service.installRoot}/releases/${m.sourceCommit}`;
    const install=plan.phases.find(phase=>phase.phase==="install-unit");
    const command=install.run[0];
    assert.match(command,/sudo -u /);
    assert.ok(install.run.slice(1).some(value=>value.includes("enable --now")));
    assert.doesNotMatch(plan.phases.find(phase=>phase.phase==="setup").run[0],/sudo -u /);
    const localRelease=root+release,envFile=root+m.service.envFile,bin=join(root,"shims");
    mkdirSync(join(localRelease,"installer"),{recursive:true});
    cpSync(new URL("../installer/lib",import.meta.url),join(localRelease,"installer/lib"),{recursive:true});
    mkdirSync(dirname(envFile),{recursive:true,mode:0o700});mkdirSync(bin);
    writeFileSync(envFile,"MURAGE_PORT=8799\nKEEP_EXISTING=retained\n",{mode:0o600});
    writeFileSync(join(bin,"ssh"),'#!/bin/sh\n[ "$1" = "--" ] && [ "$2" = "pilot-host" ] || exit 97\nshift 2\nexec /bin/sh -c "$*"\n',{mode:0o700});
    writeFileSync(join(bin,"timeout"),'#!/bin/sh\nshift\nexec "$@"\n',{mode:0o700});
    writeFileSync(join(bin,"sudo"),'#!/bin/sh\n[ "$1" = "-u" ] && [ "$2" = "murage" ] || exit 98\nshift 2\nexec "$@"\n',{mode:0o700});
    const local=command.replaceAll(m.service.installRoot,root+m.service.installRoot).replaceAll(dirname(m.service.envFile),root+dirname(m.service.envFile)).replaceAll(m.runtime.nodePath,process.execPath);
    const run=()=>spawnSync("/bin/sh",["-c",local],{env:{PATH:`${bin}:/usr/bin:/bin`},encoding:"utf8",timeout:10000});
    let result=run();assert.equal(result.status,0,`${result.stdout}${result.stderr}`);
    // Fresh process uses the same reader as installer start, not a test parser.
    const read=()=>JSON.parse(execFileSync(process.execPath,["--input-type=module","-e","const {readEnvFile}=await import(process.argv[1]);console.log(JSON.stringify(readEnvFile(process.argv[2])))",`${localRelease}/installer/lib/env-file.mjs`,envFile],{encoding:"utf8"}));
    assert.deepEqual(read(),{MURAGE_PORT:"8799",KEEP_EXISTING:"retained",MURAGE_STATIC_DIR:`${localRelease}/dist`,MURAGE_FUIGO_DIR:`${localRelease}/dist-native/fuigo/linux-x64`});
    assert.equal(statSync(envFile).mode&0o777,0o600);
    assert.equal(statSync(envFile).uid,process.getuid());
    const malformed="not a valid env line\n";writeFileSync(envFile,malformed);
