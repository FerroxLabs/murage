...
const prefix = "murage-cua-x11-input-";
...
const title = `Murage CUA input safety ${process.pid}`;
const xev = spawn(
  "xev",
...
try {
  ...
  await rpc("initialize", {
    ...
    capabilities: {},
    clientInfo: { name: "murage-x11-input-smoke", version: "1" },
  });
  ...
  const session = `murage-x11-smoke-${process.pid}`;
  assertToolResult(
