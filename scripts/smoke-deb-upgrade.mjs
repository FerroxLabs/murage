...
try {
  const status = execFileSync("dpkg-query", ["-W", "-f=${db:Status-Abbrev}", "murage"], {
    encoding: "utf8",
    stdio: ["ignore", "pipe", "ignore"],
  }).trim();
  if (status.startsWith("ii")) fail("refusing to replace a pre-existing Murage installation");
} catch (error) {
  if (String(error?.message ?? error).includes("refusing to replace")) throw error;
}

const temporary = fs.mkdtempSync(path.join(path.resolve(runnerTemp), "murage-deb-upgrade-"));
...
const legacyApp = path.join(legacyRoot, "opt", "Murage");
const legacyResources = path.join(legacyApp, "resources");
const legacyDeb = path.join(temporary, "murage_0.1.7_amd64.deb");
...
try {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      "Version: 0.1.7",
      "Architecture: amd64",
      "Maintainer: Murage CI <ci@murage.invalid>",
      "Description: Legacy Murage directory-mode upgrade fixture",
      "",
    ].join("\n"),
    { mode: 0o644 },
  );
  ...
  for (const directory of ["/opt/Murage", "/opt/Murage/resources"]) {
  ...
  for (const directory of [
    "/opt/Murage",
    "/opt/Murage/resources",
    "/opt/Murage/resources/cua-linux-x64",
  ]) {
  ...
  for (const executable of ["cua-driver", "cua-cursor-theme"]) {
    const file = path.join("/opt/Murage/resources/cua-linux-x64", executable);
  ...
  const chromiumSandbox = "/opt/Murage/chrome-sandbox";
  ...
  const installedVersion = execFileSync("dpkg-query", ["-W", "-f=${Version}", "murage"], {
    encoding: "utf8",
  }).trim();
  console.log(
