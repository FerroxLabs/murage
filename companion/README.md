...
Murage's harness listens on `127.0.0.1` and nothing else, which is the
...
`MURAGE_COMPANION_NAME` overrides a name the sidecar otherwise asks the harness
for at startup — the profile from onboarding, as *"Ada's computer"*. It falls
back to `Murage` when the harness is not up or has no profile. Read once
...
it (`MURAGE_WEBHOOK_PORT`). The companion refuses to start on either and says
