...
describe("the sidecar in front of an unmodified harness", () => {
  ...
  it("requires a paired token", async () => {
    ...
    expect((await device("GET", "/api/bots", { token: "murage_wrong" })).status).toBe(401);
  ...
  it("serves a minimal, non-cacheable companion health identity", async () => {
    ...
    expect(health.body).toEqual({ app: "murage" });
...
describe("live companion endpoint refresh", () => {
  it("requires a valid paired bearer and reflects hosted add/remove without restart", async () => {
    ...
    try {
      ...
      endpoints = [
        { kind: "hosted", priority: 0, url: "https://c-opaque.murage.test" },
...
describe("pairing, end to end", () => {
  it("turns a QR credential into a working device token", async () => {
    ...
    const control = createControlServer({
      devices: registry,
      companionPort: port,
      discovery: () => ({ advertising: false, name: "Murage" }),
    ...
    try {
      ...
      expect(opened.token).toMatch(/^murage_pair_[A-Za-z0-9_-]{43}$/);
      ...
      expect(body.token).toMatch(/^murage_/);
      ...
      expect(body.hosts).toEqual(["macbook.tail1234.ts.net", "192.168.1.42", "murage-abcd1234.local"]);
