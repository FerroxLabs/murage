...
beforeAll(async () => {
  dir = mkdtempSync(join(tmpdir(), "murage-webhook-ingress-"));
...
describe("webhook-only ingress", () => {
  it("exposes health but nothing from the main Murage API", async () => {
    const health = await fetch(`${ingress.baseUrl}/health`);
    expect(health.status).toBe(200);
    expect(await health.json()).toEqual({ app: "murage-webhooks", ready: true });
