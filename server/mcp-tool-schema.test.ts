...
// (upstream #1390). Strict providers (OpenAI and Flux-routed OpenAI-
// compatible ones, xAI, Bedrock, Kimi) reject the root anyOf whose branches
// carry no `type: "object"`. Ported from upstream 5ef9a59e.
...
describe("providerSafeInputSchema", () => {
  ...
  // An awkward schema a driver could ship: no root type, a union type, and
  // a root oneOf plus not that no strict provider accepts. Everything a
  // model needs to call the tool survives; nothing a provider refuses does.
  it("repairs an awkward root: missing type, type array and unsupported root keywords at once", () => {
    const awkward = {
      $schema: "https://json-schema.org/draft/2020-12/schema",
      description: "Scroll the page or an element.",
      properties: { direction: { type: ["string", "null"], enum: ["up", "down", null] }, amount: { type: "integer" } },
      oneOf: [
        { properties: { selector: { type: "string" } }, required: ["selector", "direction"] },
        { properties: { x: { type: "number" }, y: { type: "number" } }, required: ["x", "y", "direction"] },
      ],
      not: { required: ["selector", "x"] },
    };
    const safe = providerSafeInputSchema(awkward);
    expect(safe).toEqual({
      $schema: "https://json-schema.org/draft/2020-12/schema",
      description: "Scroll the page or an element.",
      type: "object",
    ...
    expect(providerSafeInputSchema({ type: ["null", "object"], anyOf: [{ required: ["a"] }, { required: ["b"] }] }))
