...
afterAll(async () => {
  ...
  delete process.env.MURAGE_COMPOSIO_API;
...
describe.sequential("Composio Sessions", () => {
  ...
  it("creates one stable per-installation session and reuses it", async () => {
    const created = await prepareProjectSession("ak_test", { userId: "murage_existing" });
    expect(created).toEqual({
      apiKey: "ak_test",
      userId: "murage_existing",
      sessionId: "trs_test",
    });
  ...
  it("names the project's own auth configs at creation and rebuilds a Session that predates them", async () => {
    ...
    sessionAuthConfigs = {};
    try {
      const current = { apiKey: "ak_test", userId: "murage_existing", sessionId: "trs_test" };
      ...
      expect(creates[0].body).toMatchObject({ user_id: "murage_existing", auth_configs: { twitter: "ac_twitter" } });
  ...
  it("rebuilds the Session and retries when a toolkit needs the project's own auth config", async () => {
    ...
    try {
      ...
      expect(cfg.composio).toMatchObject({ userId: "murage_existing", sessionId: "trs_test" });
    } finally {
      customAuthConfigs = [];
      sessionAuthConfigs = {};
    }
