...
const envelope = (payload: Record<string, string>) => `<murage-goal>${JSON.stringify(payload)}</murage-goal>`;
...
beforeAll(async () => {
  home = mkdtempSync(join(tmpdir(), "murage-goal-wait-cap-"));
