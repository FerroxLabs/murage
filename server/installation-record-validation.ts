...
import { DELEGATION_OUTCOMES, ROUTINE_RUN_ON, ROUTINE_RUN_STATUSES, ROUTINE_RUN_TRIGGERS, ROUTINE_TARGETS, WEBHOOK_ATTEMPT_OUTCOMES } from "../shared/record-values.ts";
...
//
// Every enumerated value comes from shared/record-values.ts, the same list the
// runtime types are derived from, so this file can never again refuse a value
// the app writes (audit A-04: a Telegram message saved a run with trigger
// "channel" and routine "telegram:<connection>", and every backup after it
// failed). Record ids restore itself relies on keep their shape; references
// to other things (a routine's source, a thread) only need to be text.
const id = z.string().regex(/^[\w-]{1,160}$/);
const reference = z.string().min(1).max(1024);
const time = z.number().finite().nonnegative();
const text = z.string();
const runOn = z.enum(ROUTINE_RUN_ON);
const target = z.enum(ROUTINE_TARGETS);
...
const routine = z.object({ id, name: text, prompt: text, botId: id, target: target.optional(), groupId: id.optional(), runOn: runOn.optional(), enabled: z.boolean(), schedule, durationMinutes: z.number().finite(), timeoutMinutes: z.number().finite().positive().optional(), attachments: z.array(attachment).optional(), sourceThreadId: reference.optional(), nextRunAt: time.nullable(), createdAt: time, updatedAt: time }).passthrough();
const run = z.object({ id, routineId: reference, routineName: text, botId: id, target: target.optional(), groupId: id.optional(), runOn: runOn.optional(), scheduledFor: z.number().finite(), status: z.enum(ROUTINE_RUN_STATUSES), manual: z.boolean(), createdAt: time, startedAt: time.optional(), finishedAt: time.optional(), sourceThreadId: reference.optional(), threadId: reference.optional(), triggerSource: z.enum(ROUTINE_RUN_TRIGGERS).optional(), attachments: z.array(attachment).optional() }).passthrough();
...
const webhookFile = z.object({ version: z.literal(1), webhooks: z.array(z.object({ id, endpointId: id, name: text, prompt: text, botId: id, runOn, enabled: z.boolean(), createdAt: time, updatedAt: time, deliveryCount: z.number().int().nonnegative(), secretHash: text.regex(/^[a-f0-9]{64}$/).optional(), verificationPending: z.boolean().optional() }).passthrough()), deliveries: z.array(z.object({ key: text.min(1), runId: id, at: time }).passthrough()), attempts: z.array(z.object({ id, webhookId: id, receivedAt: time, outcome: z.enum(WEBHOOK_ATTEMPT_OUTCOMES), statusCode: z.number().int().min(100).max(599), runId: id.optional() }).passthrough()).optional() }).passthrough();
const delegationReceipts = z.array(z.object({ id, sourceThreadId: id, toBotId: id, toBotName: text, status: z.enum(DELEGATION_OUTCOMES), result: text.optional(), finishedAt: time }).passthrough());
...
let current = "";
function fail(): never { throw new InstallationSnapshotError("INVALID_INSTALLATION_RECORDS", { path: current }); }
