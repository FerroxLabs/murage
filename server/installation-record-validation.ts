...
const delegationReceipts = z.array(z.object({ id, sourceThreadId: id, toBotId: id, toBotName: text, status: z.enum(["done", "failed", "denied", "expired", "cancelled", "busy_gave_up", "dropped", "error"]), result: text.optional(), finishedAt: time }).passthrough());
