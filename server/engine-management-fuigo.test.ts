...
async function fixture(custom = false, platform: NodeJS.Platform = "darwin") {
  const root = await mkdtemp(join(tmpdir(), "murage-fuigo-manager-")); roots.push(root);
  let version = "1.0.9", busy = false;
  const bundledCli = join(root, "bundled-fuigo"); await writeFile(bundledCli, nativeFixtureAsset().binary);
  ...
  const fetcher = vi.fn(async (url: string | URL | Request) => {
    const asset = nativeFixtureAsset(version);
  ...
  const manager = new EngineManager({ root, getInstance: async id => id === "fuigo" ? instance : undefined, isBusy: () => busy, activate, platform, arch: "arm64", fetch: fetcher as typeof fetch, run, fuigoProbe: probe });
...
it.each(["linux", "win32"] as const)("keeps %s updater isolation explicitly unqualified and refuses all selection actions", async platform => {
