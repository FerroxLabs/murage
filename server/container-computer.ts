...
//
// Murage owns only the sandbox boundary: image preparation, container
...
export const IMAGE_REPOSITORY = "localhost/murage/cua-local-vm";
export const IMAGE_LAYER_VERSION = "4";
export const IMAGE_LAYER_LABEL = "com.murage.image-layer";
export const IMAGE = `${IMAGE_REPOSITORY}:driver-${CUA_DRIVER_VERSION}-v${IMAGE_LAYER_VERSION}`;
export const CONTAINER = "murage-computer";
export const MANAGED_LABEL = "com.murage.local-vm";
export const DRIVER_LABEL = "com.murage.cua-driver";
export const BASE_IMAGE_LABEL = "com.murage.cua-base";
export const WORKSPACE_LABEL = "com.murage.workspace";
export const TARGET_LABEL = "com.murage.local-vm-target";
...
export const CUA_SOCKET = "/run/user/1000/murage-cua.sock";
export const CUA_EXECUTABLE = "/usr/local/libexec/murage/cua-driver";
...
RUN printf '%s\\n' \\
      ...
      > /usr/local/bin/prepare-murage-workspace.sh \\
    && chmod 0755 /usr/local/bin/prepare-murage-workspace.sh
RUN printf '%s\\n' \\
      '#!/bin/sh' \\
      '/usr/local/bin/prepare-murage-workspace.sh' \\
      'attempt=0' \\
      ...
      > /usr/local/bin/start-murage-cua-driver.sh \\
    && chmod 0755 /usr/local/bin/start-murage-cua-driver.sh
RUN printf '%s\\n' \\
      '' \\
      '[program:murage-cua-driver]' \\
      'command=/usr/local/bin/start-murage-cua-driver.sh' \\
      'user=cua' \\
...
export async function containerComputerStatus(
  ...
  try {
    ...
    // The prepared Murage derivative has not been built yet.
  }
  ...
  if (canProbe) {
    try {
      ...
      const readinessShot = "/tmp/murage-readiness.png";
      await runner(
        status.runtime,
        cuaExecArgs([
          "call",
...
async function prepareManagedImage(runtime: Runtime, runner: CommandRunner): Promise<void> {
  ...
  const context = await mkdtemp(join(tmpdir(), "murage-cua-image-"));
  try {
...
export async function containerComputerScreenshot(
  ...
  try {
    const screenshot = "/tmp/murage-preview.png";
    await runner(
      status.runtime,
      cuaExecArgs([
        "call",
...
export function containerComputerMcp(
  ...
  return {
    ...
    env: {
      ...
      ...(control ? { MURAGE_CONTROL_URL: control.url, MURAGE_CONTROL_TOKEN: control.token } : {}),
    },
  };
}
...
/** Cloud boxes still use Murage's high-latency REST adapter. Local VMs
...
export function computerProxyEnv(
  ...
  return {
    MURAGEBOX_BOX_ID: computer.boxId ?? "",
    MURAGEBOX_BOX_TOKEN: computer.token ?? "",
    ...(computer.control
      ? { MURAGE_CONTROL_URL: computer.control.url, MURAGE_CONTROL_TOKEN: computer.control.token }
      : {}),
  };
}
