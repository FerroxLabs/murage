...
const bindingSchema=z.object({version:z.literal(1),label:inputSchema.shape.label,target:resticS3TargetSchema,credentials:resticS3CredentialsSchema,passwordRef:reference.optional(),automatic:automaticSchema.optional(),maintenanceCredentials:resticS3CredentialsSchema.optional()}).strict();
...
export interface BackupRemoteHostOptions {
 ...
 createAdapter:(binding:Readonly<{target:ResticS3Target;credentials:ResticS3Credentials;passwordRef:string;maintenanceCredentials?:ResticS3Credentials}>)=>RemoteAdapter;
...
export function createBackupRemoteHost(options:BackupRemoteHostOptions){
 ...
 function adapter(binding:Binding){
  ...
  return options.createAdapter({target:binding.target,credentials:binding.credentials,passwordRef:binding.passwordRef,...(binding.maintenanceCredentials?{maintenanceCredentials:binding.maintenanceCredentials}:{})});
 }
 async function status():Promise<BackupRemoteStatus>{
  ...
  try{
   ...
   const common={supported:true,pending,configured:true,label:binding.label,remoteRef:binding.target.remoteRef,revision:binding.target.revision,passwordSelected:!!binding.passwordRef,maintenanceSelected:!!binding.maintenanceCredentials,automaticUpload};
   ...
   return{...common,state:saved.state,...(saved.state==="connected"?{repositoryId:saved.repositoryId}:{}),...(lastUpload?{lastUpload}:{}),...(retention?{retention}:{})};
 ...
 async function saveMaintenanceCredentials(remoteRef:unknown,expectedRevision:unknown,credentials:unknown){return exclusive(async()=>{
  ...
  await options.updateProtected(current=>{
   ...
   if(parsed.data.accessKeyId===binding.credentials.accessKeyId)refuse("BACKUP_REMOTE_INPUT_INVALID");
 ...
 async function retentionContext(remoteRef:unknown,expectedRevision:unknown){
  if(!reference.safeParse(remoteRef).success)refuse("BACKUP_REMOTE_CHANGED");
  const binding=readBinding(await options.readProtected());check(binding,expectedRevision,remoteRef);if(!binding)refuse();
  if(!binding.maintenanceCredentials)refuse("BACKUP_REMOTE_MAINTENANCE_REQUIRED");
 ...
 return{status,save,selectRepositoryPassword,saveMaintenanceCredentials,connect,uploadLatest,setAutomaticUpload,runAutomaticUpload,reconcileLatest,listBackups,downloadBackup,previewRetention,applyRetention,clearRetentionReview,isPending:()=>pending};
}
