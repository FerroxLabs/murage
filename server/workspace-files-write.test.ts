...
function fixture() {
  ...
  const save = (relativePath: string, content: string, baseRevision: FileRevision | null, extra: Record<string, unknown> = {}, hooks?: WorkspaceWriteHooks): SaveReceipt =>
...
const codeOf = (action: () => unknown) => refusal(action).code;
...
describe("bounded workspace read", () => {
  it("reports BOM, newline style and the discovery revision, and leaves the bytes untouched", () => {
    const f = fixture();
  ...
  it("classifies newline styles and keeps a second BOM in the content", () => {
  ...
  it("refuses non-UTF-8, oversized, linked, hard-linked, private, hidden and missing files", () => {
    const f = fixture();
    f.put("latin1.md", Buffer.from([0x63, 0x61, 0x66, 0xe9]));
    expect(codeOf(() => f.read("latin1.md"))).toBe("unsupported-encoding");
    f.put("big.md", Buffer.alloc(WORKSPACE_TEXT_MAX_BYTES + 1, 0x61));
    expect(codeOf(() => f.read("big.md"))).toBe("too-large");
    ...
    expect(codeOf(() => f.read("link.md"))).toBe("linked-file");
    symlinkSync(outside, join(f.taskRoot, "linked"));
    expect(codeOf(() => f.read("linked/secret.md"))).toBe("linked-file");
    const hard = f.put("hard.md", "shared");
    linkSync(hard, join(f.taskRoot, "hard-copy.md"));
    expect(codeOf(() => f.read("hard.md"))).toBe("not-regular-file");
    mkdirSync(join(f.taskRoot, "folder.md"));
    expect(codeOf(() => f.read("folder.md"))).toBe("not-regular-file");
    f.put("MEMORY.md", "private");
    expect(codeOf(() => f.read("MEMORY.md"))).toBe("private-file");
    expect(codeOf(() => f.read("memory/notes.md"))).toBe("private-file");
    for (const bad of [".hidden.md", "../x.md", "/etc/hosts", "a\\b.md", "c:x.md", ""]) expect(codeOf(() => f.read(bad)), bad).toBe("invalid-path");
    expect(codeOf(() => f.read("missing.md"))).toBe("not-found");
    expect(codeOf(() => f.read("missing/deeper.md"))).toBe("not-found");
  });

  it("answers the conversation state instead of reading legacy, unknown or not-yet-created workspaces", () => {
    const f = fixture();
    f.put("a.md", "x");
    f.task.cwd = null;
    expect(codeOf(() => f.read("a.md"))).toBe("no-dedicated-workspace");
    f.task.cwd = undefined; rmSync(f.taskRoot, { recursive: true });
    expect(codeOf(() => f.read("a.md"))).toBe("not-found");
    expect(codeOf(() => readWorkspaceFile(f.deps, { scope: { botId: "nobody", threadId: "thread" }, relativePath: "a.md" }))).toBe("scope-unavailable");
  });
...
describe("revision-conditioned Markdown write", () => {
  it("writes the exact bytes, keeps BOM, CRLF and file mode, and keeps the previous revision in Files", () => {
    const f = fixture();
    ...
    const second = f.save("reports/report.md", "# Newer\n", receipt.revision);
    expect(readFileSync(path, "utf8")).toBe("# Newer\n");
    expect(second.previousRevision).toBe(receipt.revision);
  });

  it("saving the unchanged text rewrites nothing and adds no saved version", () => {
    const f = fixture();
    ...
    const receipt = f.save("same.md", opened.content, opened.revision, { bom: opened.bom });
  ...
  it("refuses a stale revision and never overwrites an external change", () => {
    const f = fixture();
    const path = f.put("a.md", "# Base\n");
    const opened = f.read("a.md");
    writeFileSync(path, "# Changed elsewhere\n");
    const error = refusal(() => f.save("a.md", "# My edit\n", opened.revision));
  ...
  it("rechecks the file immediately before the commit", () => {
    const f = fixture();
    const path = f.put("a.md", "# Base\n");
    const opened = f.read("a.md");
  ...
  it("refuses a file that was moved, deleted or swapped for a link", () => {
    const f = fixture();
    const path = f.put("a.md", "# Base\n");
    const opened = f.read("a.md");
    unlinkSync(path);
    expect(codeOf(() => f.save("a.md", "# Edit\n", opened.revision))).toBe("not-found");
  ...
  it("Save a copy creates a new private file and never replaces an existing one", () => {
    const f = fixture();
    const receipt = f.save("copy.md", "# Copy\n", null);
    ...
    expect(codeOf(() => f.save("copy.md", "# Other\n", null))).toBe("already-exists");
    expect(readFileSync(path, "utf8")).toBe("# Copy\n");
    // Created by someone else between the check and the commit.
    const racer = join(f.taskRoot, "race.md");
    expect(codeOf(() => f.save("race.md", "# Mine\n", null, {}, { beforeCommit: () => writeFileSync(racer, "# Theirs\n") }))).toBe("already-exists");
    expect(readFileSync(racer, "utf8")).toBe("# Theirs\n");
    // No folders are created for a copy.
    expect(codeOf(() => f.save("new-folder/copy.md", "# Copy\n", null))).toBe("not-found");
    expect(f.leftovers()).toEqual([]);
  });

  it("writes Markdown only, within the size bound, from a well-formed request", () => {
    const f = fixture();
    const path = f.put("a.txt", "text"), opened = f.read("a.txt");
    expect(codeOf(() => f.save("a.txt", "changed", opened.revision))).toBe("invalid-request");
    expect(readFileSync(path, "utf8")).toBe("text");
    const md = f.put("a.md", "x"), base = f.read("a.md").revision;
    expect(codeOf(() => f.save("a.md", "a".repeat(WORKSPACE_TEXT_MAX_BYTES + 1), base))).toBe("too-large");
    expect(codeOf(() => f.save("a.md", "a".repeat(WORKSPACE_TEXT_MAX_BYTES - 2), base, { bom: true }))).toBe("too-large");
    expect(codeOf(() => f.save("a.md", "broken \uD800 surrogate", base))).toBe("invalid-request");
    expect(codeOf(() => f.save("MEMORY.md", "x", null))).toBe("private-file");
    ...
    ]) expect(codeOf(() => writeWorkspaceMarkdown(f.deps, bad)), JSON.stringify(bad)).toBe("invalid-request");
    expect(readFileSync(md, "utf8")).toBe("x");
    // A full-size document is accepted.
    const full = "b".repeat(WORKSPACE_TEXT_MAX_BYTES);
    expect(f.save("a.md", full, base).bytes).toBe(WORKSPACE_TEXT_MAX_BYTES);
    expect(statSync(md).size).toBe(WORKSPACE_TEXT_MAX_BYTES);
  });

  it("refuses legacy, remote and not-yet-authorized workspaces", () => {
    const f = fixture();
    f.put("a.md", "x");
    const base = f.read("a.md").revision;
    f.task.cwd = null;
    expect(codeOf(() => f.save("a.md", "y", base))).toBe("no-dedicated-workspace");
    writeFileSync(join(f.dataDir, "routines.json"), JSON.stringify({ runs: [{ botId: "bot", threadId: "thread", runOn: "cloud" }] }));
    expect(codeOf(() => f.save("a.md", "y", base))).toBe("remote-workspace");
    ...
    expect(codeOf(() => f.save("a.md", "y", base))).toBe("scope-unavailable");
    expect(codeOf(() => f.save("new.md", "y", null))).toBe("scope-unavailable");
    expect(readFileSync(join(f.taskRoot, "a.md"), "utf8")).toBe("x");
  });
});

describe("bot-active hold", () => {
  it("holds an overwrite while a bot turn is using the workspace, and still allows Save a copy", () => {
    const f = fixture();
    const path = f.put("a.md", "# Base\n");
    const opened = f.read("a.md");
    f.leases.acquireWriter("turn-1", f.taskRoot);
    const error = refusal(() => f.save("a.md", "# Edit\n", opened.revision));
    expect([error.code, error.status]).toEqual(["bot-writing", 423]);
    expect(readFileSync(path, "utf8")).toBe("# Base\n");
    expect(f.artifactCount()).toBe(0);
    expect(f.leftovers()).toEqual([]);
    expect(f.save("a copy.md", "# Edit\n", null).previousRevision).toBeNull();
  ...
  it("keeps a bot turn from starting inside the commit window and releases the hold afterwards", () => {
    const f = fixture();
    f.put("a.md", "# Base\n");
    let refused: unknown;
  ...
  it("fails closed when the writer registry is not available", () => {
    ...
    expect(codeOf(() => writeWorkspaceMarkdown(deps, request))).toBe("bot-writing");
    expect(readFileSync(path, "utf8")).toBe("# Base\n");
    expect(writeWorkspaceMarkdown(deps, { ...request, relativePath: "copy.md", baseRevision: null }).previousRevision).toBeNull();
  });
});

describe("previous revision must be kept", () => {
  it("overwrites nothing when the Files library cannot store the previous revision", () => {
    const f = fixture();
    const path = f.put("a.md", "# Base\n");
    const opened = f.read("a.md");
    ...
    const full = refusal(() => f.save("a.md", "# Edit\n", opened.revision));
...
describe("save version", () => {
  it("copies exactly the chosen revision into Files without invented provenance, once", () => {
    const f = fixture();
  ...
  it("refuses a changed, private, oversized or malformed request", () => {
    const f = fixture();
    ...
    expect(error.currentRevision).toBe(f.read("a.md").revision);
    f.put("MEMORY.md", "private");
    expect(codeOf(() => saveWorkspaceVersion(f.deps, { scope: f.scope, relativePath: "MEMORY.md", revision }))).toBe("private-file");
    ...
    expect(codeOf(() => saveWorkspaceVersion(f.deps, { scope: f.scope, relativePath: "huge.bin", revision: huge }))).toBe("too-large");
