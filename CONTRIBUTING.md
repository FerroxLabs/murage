...
```sh
git clone https://github.com/FerroxLabs/murage && cd Murage
pnpm install
...
- stable `Murage-amd64.deb` and `Murage.AppImage` copies used by the latest-download links;
...
[`murage-releases`](https://github.com/FerroxLabs/murage-releases) repository. Then verify the checksum
...
Data lives in `~/.murage/` (bots, transcripts, per-thread NDJSON event logs, config with keys).
...
- **Never touch the real `~/.murage`.** The setup file points `HOME` at a temp dir; keep it
  that way.
...
API keys are write-only: they land in `~/.murage/config.json` via `PUT /api/config` and the API
