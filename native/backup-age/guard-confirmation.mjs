...
const root = dirname(fileURLToPath(import.meta.url)), directory = join(root, "guard-confirmation-20260913-154251");
...
try {
  ...
  const helperSha256 = hash(helper); record("helper-identity", { helperSha256 }); assert.equal(helperSha256, "ef3c7037e7c0619dfb1b50449c651b6cdff069b9e6600c0723fac0b5f23e0c90");
  ...
  assert.equal(results.length, 8, "all eight guard probes required");
  assert.equal(integrity, true, "original file and ancestor identity/hash required");
  assert.equal(results.filter(r => r.allowed !== r.expectedAllowed).length, 0, "all guard expectations must hold");
  ...
  writeFileSync(join(root, "guard-confirmation-receipts.json"), JSON.stringify(receipts, null, 2) + "\n");
}
