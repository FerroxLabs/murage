...
$taskName = 'Murage-B20-GuardConfirmation-20260913-154251'
...
if (Test-Path "$scratch\guard-confirmation-invoked.txt") { throw 'Diagnostic already invoked; no replay' }
$arguments = '-NoProfile -NonInteractive -WindowStyle Hidden -Command "& ''{0}\native-probe.exe'' --guard-diagnostic *> ''{0}\guard-confirmation.log''; exit $LASTEXITCODE"' -f $scratch
...
Export-ScheduledTask -TaskName $taskName -TaskPath '\' | Set-Content -LiteralPath "$scratch\guard-confirmation-task.xml"
[DateTime]::UtcNow.ToString('o') | Set-Content -LiteralPath "$scratch\guard-confirmation-invoked.txt"
