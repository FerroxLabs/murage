...
const routineEventSchema = z.discriminatedUnion("source", [
  ...
  z.object({ ...common, source: z.literal("channel"), origin: z.object({ kind: z.literal("channel"), channel: z.literal("telegram"), connectionId: id }).strict() }).strict(),
]);
...
export function createRoutineEvent(input: { runId: string; definitionId: string; receivedAt: number; source: RoutineEvent["source"]; webhookId?: string; telegramConnectionId?: string }): RoutineEvent {
  return routineEventSchema.parse({ version: 1, id: input.runId, definitionId: input.definitionId, receivedAt: input.receivedAt,
    budgetId: input.runId, source: input.source, origin: input.source === "channel" ? { kind: "channel", channel: "telegram", connectionId: input.telegramConnectionId }
...
export function routineEventForRun(run: { id: string; routineId: string; createdAt: number; scheduledFor: number; triggerSource?: RoutineEvent["source"]; manual: boolean; webhookId?: string; telegramConnectionId?: string; event?: unknown }): RoutineEvent | undefined {
  const source = run.telegramConnectionId ? "channel" : run.triggerSource === "webhook" || run.webhookId ? "webhook"
  ...
  const expected = parseRoutineEvent({ version: 1, id: run.id, definitionId: run.routineId,
    ...
    origin: source === "channel" ? { kind: "channel", channel: "telegram", connectionId: run.telegramConnectionId }
