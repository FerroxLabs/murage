...
//   5. SECOND PASS, this one measured rather than read. Steps 1-4 left 1,872
//      terms across 129 slugs, and the two headline bugs they were cut for
//      were fixed while ~57 more of the same class survived — because a term
//      is only safe if a plausible EVERYDAY sentence containing it should
//      reach that profile, and nothing in steps 1-4 asked that question.
//
//      HOW THE SET WAS RE-DERIVED. Every term on a LIVE slug (one whose
//      declared skills resolve on disk, so `intakeProfileAt` can offer it —
//      110 of the 129) was probed through the real classification path as
//      `"<term> qqqq"`: two topic tokens, of which only the term can hit
//      anything. That is exactly the strong-tier shape of any everyday
//      sentence whose only vocabulary word is that term. 559 of the 849 live
//      terms produced a ONE-PRESS CONFIRM card on their own; 290 produced a
//      two-chip narrow pick. Each was then judged by eye against its profile,
//      on one rule:
//
//        REMOVE when the profile's own sense of the word is JARGON, a
//        METAPHOR, or SHAPE PROSE and the everyday sense is a different
//        literal thing — `spin` (SPIN selling / a washing machine), `door`
//        (fake-door test / a front door), `health` (account health / a
//        person's), `hygiene` (CRM / dental), `unit` (unit economics / a
//        broken one), `mix` (channel mix / a cake), `scan` (AI detection /
//        an MRI), `matter` (front matter / what's the matter). Also removed:
//        roster listings, where a launcher's terms name its MEMBERS rather
//        than its subject (`lens`, `mend`, `patch`, `sentry`, `smith`), and
//        tokenizer debris that survived step 2 (`anti` from "anti-guru",
//        `opt` from "opt-in", `multi` from "multi-touch").
//
//        KEEP when the ordinary word is the profile's literal subject, even
//        though it collides — `paper`, `course`, `trade`, `invoice`, `human`,
//        `balance`, `test`, `plot`, `apology`, `office`, `traffic`, `store`.
//        Removing these breaks the sentence the list exists for, which is a
//        worse failure than a question.
//
//      264 terms went, across 92 slugs, leaving 1,608 across 129 (858
//      unique). Every removal was re-probed afterwards: none of the 264 was
//      PREFIX-SHADOWED by a sibling that survived it (`sales`/`sale`,
//      `testing`/`test`, `humanizer`/`human`), which is the way this edit
...
export const INTAKE_MATCH_TERMS: Readonly<Record<string, string>> = {
  ...
  helm: "ben cadence coach coaching decision founder horowitz leadership management thinking unstuck",
  humanizer: "detection human humanise humanize humanizer plagiarism prose rewrites tone",
  lens: "analyst analytics cohort conversion dashboard experiment funnel kaushik kohavi kpi measurement metrics research retention",
  mend: "churn customer lincoln murphy onboarding outcome prevention renewal retention success support ticket triage",
  mira: "brand branding design identity logo marty neumeier positioning visual",
  patch: "admin cadence crm documentation harnish operations ops process processes scaling sop sops verne workflow",
  probe: "eric experiment hypothesis mvp prototype quantitative research ries test testing tests validate validation validator",
  "quiet-money-career-strategist": "auditor career comp compensation equity income job jurisdiction market money negotiation offer position quiet raise salary strategist value",
  ...
  "quiet-money-spending-auditor": "audit auditor budget expenses lifestyle money position quiet spend spending subscriptions",
  "quiet-money-time-coach": "audit balance coach freedom income lifestyle money overwork position quiet wealth",
  "quiet-money-windfall-navigator": "divorce income inheritance money navigator payout redundancy windfall windfalls",
  "quiet-money": "coach debt finance financial guru math money personal quiet savings wealth",
  research: "bob competitor competitors customer interview interviews moesta persona product research segmentation survey",
  sales: "closing deal deals discovery objection objections pipeline prospect prospects quota sale sales sell selling",
  sentry: "business compliance contract contracts corp counsel formation incorporation lawyer legal llc privacy startup terms trademark",
  slate: "candidate claire contractor employee employees evaluation hire hiring hughes interview job johnson onboarding outcome recruit recruiting scaling staff structured talent",
  smith: "adrs api architect architecture backend code coding developer engineering frontend ryan software specs technical",
  spark: "architecture book course curriculum education learner lesson mctighe students syllabus teaching wiggins",
  stage: "andy deck decks demo fundraising investor narrative pitch presentation raskin sell sequoia storytelling strategic",
  vault: "conversion discovery dtc ecommerce listings marketplace optimization optimize page product sell store storefront",
  verdict: "content critique edit editing proofreading quality review rewrite",
  voiceprint: "brand style tone voice voiceprint writing",
  "academic-paper": "academic bibliography citation citations creator dissertation docx equations journal latex paper papers references research scholarly technical thesis",
  ...
  "beautiful-mermaid": "ascii chart diagram diagrams flowchart mermaid svg visual",
  "book-copy-editor": "book consistency copy copyedit editor grammar manuscript proofread publishing",
  "book-developmental-editor": "architect book chapter chapters developmental editing editor editorial manuscript publishing",
  "book-nonfiction-architect": "architect book chapter chapters fiction nonfiction outline publishing",
  "book-production": "book copy kdp manuscript production publishing typesetting",
  ...
  "excel-creator": "creator excel formulas officecli spreadsheet spreadsheets workbook xlsx",
  explainer: "concept concepts explain explainer explanation learn learning teach teaching tutorial",
  "financial-model-creator": "assumptions business cashflow creator excel financial forecast formula model models projections prompts spreadsheets valuation",
  ...
  "human-3-coach": "burnout coach coaching dan discipline habits holistic human koe mindset motivation personal procrastination purpose vocation wellbeing",
  ignition: "beginner business ignition income landing page website",
  ...
  "story-roleplay": "character creative fiction narrative roleplay sillytavern story",
  ...
  writer: "article blog drafting editing prose rewrite writer writing",
  ...
  "ad-account-mechanic": "account ads advertising analyst campaign channels copy creative facebook google offer ops paid roas sell",
  "affiliate-site-engine": "affiliate analyst blog channels content copy niche research sell seo site",
  "back-office-crew": "admin analyst inbox inventory numbers office operations ops orders store support",
  "bootstrap-profit": "bootstrap bootstrapping cashflow channels numbers offer profit profitability revenue sales sell",
  "caption-carousel-studio": "brand caption carousel copy cta discovery instagram post posts research sales slides social studio",
  "cohort-ops-control-tower": "analyst channels cohort comms community copy course ops program risk students",
  ...
  "customer-success-org": "accounts audit churn churning customer org renewal renewals retention success support",
  "daily-briefing": "briefing coach digest numbers sales standup",
  "damage-control": "apology backlash brand comms copy counsel crisis damage legal reputation research",
  "dev-shop": "audit code cto dev developer engineering features founders garry gstack releases repo shipping software standups workflow",
  ...
  "growth-loop": "acquisition analyst channels funnel growth research retention sell",
  "hook-lab": "brand copy hook hooks reels research script shorts video",
  ...
  "info-product-launch": "assembles book brand channels copy course digital launch product sell",
  "lead-gen-outbound": "copy gen icp lead leads outbound outreach personalized prospect prospecting research sales sell sequence squad support",
  "lead-magnet-forge": "brand channels copy email forge freebie landing lead magnet optin research",
  "lesson-scripting-crew": "coach copy course humanizer lesson script scripting teaching video",
  "link-disclosure-custodian": "affiliate analyst code compliance counsel custodian disclosure ftc link links site",
  "listing-forge": "amazon copy description etsy forge listing listings platform product research sell shopify storefront",
  "local-seo-reviews": "analyst business copy engine google local maps optimized pages research reviews sell seo support visibility",
  "marketing-agency": "agency audit beacon campaigns channels clients cmo marketing mira quill scout sell",
  ...
  "paid-ads-war-room": "ads advertising analyst budget campaign channels copy launch offer paid roas sell",
  "pre-mortem-room": "adversarial audit buyer channel failure launch mortem pre risk risks writer",
  "pricing-tribunal": "adversarial anchor churn discount offer packaging price pricing rewritten sell tiers tribunal value",
  "product-launch": "assembles brand channels copy launch market product release research sales sell",
  "promo-calendar-war-room": "calendar channels copy deal discount ops promo promotion research sale sell",
  "quiet-money-council": "auditor career coach council finance generational money navigator numbers personal planner position quiet specialists spending strategist wealth windfall",
  "quiet-money-standing": "audit coach finance money personal quiet savings spending",
  ...
  "reply-desk": "comments copy dms engagement inbox leads replies reply research sales support",
  "repurpose-engine": "channels clips content copy humanizer platform posts recording repurpose repurposing research shorts",
  "review-article-factory": "affiliate article copy counsel disclosure factory links product publish research review roundup",
  "review-engine": "analyst copy digest feedback ratings replies reviews sequence solicitation support testimonials",
  "saas-mvp-sprint": "app brand code copy landing launch mvp page pipeline prototype saas software validation validator visual",
  "sales-org": "audit crm deals org outbound pipeline quota sales scout sell sequences",
  "sales-pipeline": "bottleneck crm deals forecast offer pipeline research sales sell",
  "seo-content-engine": "analyst articles blog calendar cluster content keyword keywords optimized publish ranking research seo",
  ...
  "student-support-desk": "coach education macros ops replies research student students support tickets tutoring",
  "support-stack": "churn copy customer helpdesk macros onboarding ops prevention service support tickets triage voice",
  "template-factory": "brand copy digital etsy factory listing printables products sales sell templates",
  "trend-desk": "brand copy newsjacking research topical trend trending trends",
  "validate-before-build": "hypothesis idea offer prototype research startup test validate validation validator",
  "validation-cell": "analyst copy idea pivot research test validate validation validator",
  "war-room": "advisors board cfo critique customer decision growth skeptic strategy voice",
  "winning-product-war-room": "analyst dossier dropshipping numbers page product research sourcing validation winning",
};
