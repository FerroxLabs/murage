...
const devices = { "/": 16777229, "/System/Volumes/Data": 16777229, "/Applications/Murage.app/Contents/MacOS/Murage": 16777229,
  ...
  "/Volumes/External/Murage.app/Contents/MacOS/Murage": 16777239, "/Volumes/External/murage-data": 16777239, "/Volumes/Second/Murage.app/Contents/MacOS/Murage": 16777300 };
...
test("the app on an external drive is refused", () => {
  assert.equal(check("/Volumes/External/Murage.app/Contents/MacOS/Murage", home), "app");
});
test("the data folder on an external drive is refused", () => {
  assert.equal(check("/Applications/Murage.app/Contents/MacOS/Murage", ["/Volumes/External/murage-data", home[1]]), "data");
});
test("both elsewhere name both, and any other volume counts, not only external ones", () => {
  assert.equal(check("/Volumes/External/Murage.app/Contents/MacOS/Murage", ["/Volumes/External/murage-data"]), "both");
...
test("other platforms and unreadable facts never block", () => {
  assert.equal(check("/Volumes/External/Murage.app/Contents/MacOS/Murage", home, { platform: "linux" }), null);
  assert.equal(check("/nowhere/Murage", ["/also/nowhere"]), null);
  assert.equal(closedVolumeProblem({ appPaths: ["/Volumes/External/Murage.app/Contents/MacOS/Murage"], dataPaths: [], platform: "darwin", device: () => { throw Error("no"); } }), null);
});
