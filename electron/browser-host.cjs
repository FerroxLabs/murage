...
// renderer, never a debugging port on Murage itself. The manager is
...
function createBrowserHost({ manager, token = randomBytes(32).toString("hex"), now = Date.now }) {
  ...
  const handle = async (req, res) => {
    ...
    if (!surface) return json(res, 503, { error: "the Murage window is closed — open it to use the browser" });
