...
test("the Ubuntu command resolves dependencies", () => {
  ...
  assert.equal(
    command,
    "sudo apt-get install -y '/home/u/.cache/murage-updater/pending/x.deb'",
  );
...
test("a staged path that is gone is not used", () => {
  const workspace = mkdtempSync(join(tmpdir(), "murage-staged-"));
  try {
    const present = join(workspace, "Murage.deb");
...
test("the quoted path survives a shell round-trip", { skip: posixShell }, () => {
  const workspace = mkdtempSync(join(tmpdir(), "murage-quote-"));
  try {
...
test("the whole command parses into the arguments apt-get would receive", { skip: posixShell }, () => {
  const file = "/home/o'brien/.cache/murage-updater/pending/Murage-0.1.44-amd64.deb";
...
test("the package marker decides the install path", () => {
  ...
  assert.equal(
    linuxPackageType({
      ...
      appImage: "/home/u/Murage.AppImage",
