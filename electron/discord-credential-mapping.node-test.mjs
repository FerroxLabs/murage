...
test("Discord credentials use canonical encrypted rows and preserve chosen identity", () => {
  ...
  for (const [name, field, env] of [["discordBotToken", "botToken", "MURAGE_DISCORD_BOT_TOKEN"]]) {
    assert.deepEqual(JSON.parse(JSON.stringify(handlers[name]("fake-secret"))), { discord: { [field]: "fake-secret" } });
    assert.deepEqual(WORKSPACE_CREDENTIALS.find(row => row.name === name), { section: "discord", field, name, env });
  }
  const migrated = migrateWorkspaceCredentials({ discord: { botToken: "fake-b", applicationId: "11" } }, { otherKey: "keep" });
  assert.deepEqual(migrated.config, { discord: { applicationId: "11" } });
  assert.deepEqual(workspaceCredentialEnv(migrated.credentials), { MURAGE_DISCORD_BOT_TOKEN: "fake-b" });
