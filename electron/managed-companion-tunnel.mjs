...
export function resolveCloudflaredBinary({
  ...
  const override = environment.MURAGE_CLOUDFLARED_PATH?.trim();
...
/** Remove only token files created by a dead Murage process. Suspicious
...
async function verifyHostedEndpoint(
  ...
  try {
    return JSON.parse(text)?.app === "murage";
  } catch {
    return false;
  }
}
