...
test("actual main remote handlers enforce arity and retain operations until settlement",async()=>{
 ...
 vm.runInNewContext(loop.getText(tree),context);assert.equal(handlers.size,9);assert.throws(()=>handlers.get("backup-remote:connect")({},"ref"),/INPUT_INVALID/);assert.throws(()=>handlers.get("backup-remote:reconcileLatest")({},"ref",1),/INPUT_INVALID/);assert.throws(()=>handlers.get("backup-remote:downloadBackup")({},"ref",1),/INPUT_INVALID/);assert.throws(()=>handlers.get("backup-remote:setAutomaticUpload")({},"ref",1),/INPUT_INVALID/);assert.equal(handlers.has("backup-remote:runAutomaticUpload"),false);
 ...
 const preload=readFileSync(new URL("./preload.cjs",import.meta.url),"utf8");for(const action of ["status","save","selectRepositoryPassword","connect","uploadLatest"])assert.ok(preload.includes(`backup-remote:${action}`));
});
