...
test("strict descriptor binds only canonical profile, owner and trigger facts",()=>{
...
test("closed argv and environments reproduce both profile selectors and refuse drift",()=>{
...
test("profile validation re-resolves default and restored selection and checks current owner/artifact",()=>{
...
test("private descriptor reader rejects links, broad permissions and oversized metadata",()=>{
...
test("per-user job definitions escape directives and never reuse sign-in/system-server jobs",()=>{
...
test("private stages are idempotent, exact and never labelled installed",()=>{
...
test("foreign stage collision is refused and narrow stage removal preserves sibling evidence",()=>{
...
test("injected install requires exact OS readback and refuses foreign definitions",async()=>{
...
test("disable precedes exact removal, never kills running work and preserves stage/receipts",async()=>{
...
test("lightweight trigger uses same coordinator hint and launches only admitted due work",async()=>{
