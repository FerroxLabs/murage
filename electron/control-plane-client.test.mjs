...
const INSTALL = `murage_install_${"a".repeat(22)}.${"b".repeat(43)}`;
...
describe("control-plane desktop client", () => {
  it("accepts exact HTTPS and loopback development origins only", () => {
    expect(normalizeControlPlaneURL("https://accounts.murage.ai/")).toBe(
      "https://accounts.murage.ai",
    );
    expect(normalizeControlPlaneURL("http://127.0.0.1:8787/")).toBe("http://127.0.0.1:8787");
    expect(normalizeControlPlaneURL("http://accounts.murage.ai")).toBe("");
    expect(normalizeControlPlaneURL("https://accounts.murage.ai/api")).toBe("");
    expect(normalizeControlPlaneURL("https://user:secret@accounts.murage.ai")).toBe("");
  });
  ...
  it("normalizes an email without accepting malformed input", () => {
    ...
    expect(normalizeControlPlaneURL({ toString: () => "https://accounts.murage.ai" })).toBe("");
  });
  ...
  it("accepts plain cross-realm response records", async () => {
    ...
    const client = createControlPlaneClient({
      baseURL: "https://accounts.murage.ai",
  ...
  it("requires the exact healthy control-plane identity before onboarding", async () => {
    ...
    const fetchImpl = vi
      .fn()
      .mockResolvedValueOnce(jsonResponse({ ok: true, service: "murage-control-plane" }))
    ...
    expect(fetchImpl.mock.calls[0][0]).toBe("https://accounts.murage.ai/healthz");
  ...
  it("identifies native Better Auth mutations with the trusted control-plane origin", async () => {
    const fetchImpl = vi.fn(async (_url, init) => {
      ...
      if (wireHeaders.has("sec-fetch-mode") && wireHeaders.get("origin") !== "https://accounts.murage.ai") {
  ...
  it("keeps a valid installation credential without rotating it", async () => {
    const fetchImpl = vi.fn(async (url) => {
      expect(url).toBe("https://accounts.murage.ai/v1/installations/self");
      return jsonResponse({
        installation: {
          id: INSTALL_ID,
    ...
    const client = createControlPlaneClient({ baseURL: "https://accounts.murage.ai", fetchImpl });
  ...
  it("recovers a lost installation credential by rotating the matching identity", async () => {
    const rotated = `murage_install_${"c".repeat(22)}.${"d".repeat(43)}`;
  ...
  it("lists validated active installations for response-loss cleanup", async () => {
    const fetchImpl = vi.fn(async (url, init) => {
      expect(url).toBe("https://accounts.murage.ai/v1/installations");
  ...
  it("validates endpoint material without leaking the connector token into the URL", async () => {
    ...
    const fetchImpl = vi.fn(async (url, init) => {
      expect(url).toBe("https://accounts.murage.ai/v1/installations/self/endpoint");
      expect(init.headers.get("authorization")).toBe(`Bearer ${INSTALL}`);
      expect(url).not.toContain(connectorToken);
      return jsonResponse({ endpoint: { url: "https://c-opaque.murage.ai" }, connectorToken });
    });
    const client = createControlPlaneClient({ baseURL: "https://accounts.murage.ai", fetchImpl });
    await expect(client.ensureEndpoint(INSTALL)).resolves.toEqual({
      endpoint: { url: "https://c-opaque.murage.ai" },
      connectorToken,
    });
  });
