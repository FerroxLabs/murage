// Local skill compiler and filesystem boundary for reviewed recordings.
//
// Split out of skill-recorder.mjs so it does not import Electron: the
// installation-selection integration node test drives this exact writer
// through the same handler main.mjs registers (0.1.52 R2-T1, audit B1).
...
function cleanText(value, max = 500) {
  // Stripping control characters from helper/renderer text is the point here.
...
// The caller names the installation root it owns. There is no environment or
// ~/.murage fallback: after a separate restore those name the retained
// original, which this process does not own (audit B1). An unusable root is
// refused before any directory is created.
function assertSkillDataRoot(dataRoot) {
  let stats = null;
  try {
    if (typeof dataRoot === "string" && path.isAbsolute(dataRoot)) stats = statSync(dataRoot);
  } catch {
    stats = null;
  }
  if (!stats?.isDirectory()) {
    throw Object.assign(new Error("This installation is not available for saving skills right now."), {
      code: "SKILL_DATA_ROOT_UNAVAILABLE",
    });
  }
}
