...
| Antigravity upstream rewrite | `6dbe5ae8` trio | **DECIDED 2026-09-04: antigravity STAYS in the fleet as an option; fuigo is primary.** The trio is DEFER (a project, not a pick): 5 tailnet-reachable POST routes, remote binary download+chmod, OAuth loopback, and an agent-driven fs read/write handler in the SHARED `acp/core.ts` fuigo runs through. Our existing `agy`-CLI driver keeps working; revisit only if Google retires that CLI path |
| MCP server management UI? | `074d2f7e` | **DECIDED 2026-09-04: TAKE-ADAPTED** — Sean: custom MCP beyond Composio is coming, and this is the framework for what migrates from Wayland. Hard condition: all six routes behind `requestSurface(...) !== "desktop"` → 404 (`index.ts:9472` precedent), or it is the rejected #681 again |
