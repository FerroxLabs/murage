...
Add OpenCode Go as a first-class Murage engine by running the maintained
OpenCode CLI over its official Agent Client Protocol (ACP) interface and selecting
models from the `opencode-go/*` provider.

The first implementation should reuse Murage's ACP runtime rather than call
...
provide inference; Murage does not currently have a provider-independent
...
- OpenCode accepts an `OPENCODE_API_KEY` environment variable. Murage should
...
the process lifecycle Murage uses for Grok and Gemini.
...
- load that session on the next Murage turn;
...
- concurrent Murage threads without shared ACP or credential state.
...
- OpenCode Go is shown as unavailable until both its CLI and credential are
  present; Murage never selects it as a runnable default otherwise.
- The setup flow works without restarting Murage and never silently executes
  an installer.
...
6. Can the existing ACP usage metadata distinguish cumulative session totals from
   per-turn usage so Murage does not double-count it?
