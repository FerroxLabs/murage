# Murage harness implementation plan
...
(pi, agent-orchestrator, deepseek-harness) and asking what Murage lacked by
...
**The reframe that reorders everything.** Murage is itself a harness. The
...
**On-disk forward compatibility.** `~/.murage` is shared across app versions.
