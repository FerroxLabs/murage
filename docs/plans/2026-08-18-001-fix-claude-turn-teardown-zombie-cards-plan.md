...
**Origin issue:** [milind-soni/Murage#211](https://github.com/milind-soni/Murage/issues/211) — "Turn teardown discards the permission broker without ensuring child CLI/MCP exit — late approval requests become dead 'zombie' cards"
...
`sendTurn()` in `server/drivers/claude.ts` spawns the `claude` CLI per turn and wires a `createPermissionBroker()` instance to a per-turn unix socket (named pipe on Windows). The CLI's own spawned MCP `muragebox` process (`server/permission-proxy.ts`) forwards `approve`/`ask_user` tool calls over that socket to the broker, which the harness renders as `request.opened` cards.
...
Rationale: the simplest fix that satisfies R2 without changing `close()`'s existing contract for in-flight `pending` asks. The `data` handler already runs inside the closure that owns `pending`, `timeoutMs`, and now a `closed` flag set at the top of `close()`; checking it before creating a new pending entry is a one-line, easily-tested guard. Closure handling takes precedence over the active-turn duplicate-ID guard. **The response must be an explicit system-source deny/answer, not a silent drop:** permissions receive `deny` with `Murage: the turn ended`; questions receive `answer` with `Murage: the turn is ending — wrap up.` `server/permission-proxy.ts`'s `waiting` map (the child-side promise the CLI's `tools/call` is awaiting) is only resolved by an incoming `{t:"answer",...}` message or by the connection's own `error`/`close` firing `dead()` — nothing in `permission-proxy.ts` times out a single ask on its own. A silent drop on the broker side leaves that specific MCP tool call hanging until something else closes the connection, which is exactly the kind of hang R2 exists to prevent.
...
- [ ] Changes committed with a message referencing milind-soni/Murage#211.
