...
The iOS app is a thin, native client for the Murage instance running on
...
The Mac must be running Murage and must not be asleep. Desktop
...
The sidecar keeps its device registry in `~/.murage/devices.json`. That is
