...
Murage is local-first by default: the harness listens on loopback, agent CLIs run on your machine, and application state lives under `~/.murage`.

## Typical self-hosted setup

1. Run Murage on your personal macOS, Windows, or Ubuntu computer.
...
Self-hosting Murage does not self-host every optional provider:
