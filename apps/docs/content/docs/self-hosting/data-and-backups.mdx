...
Then choose **Create off-site password**. Murage makes a long random password that locks the off-site copy and saves it as a file next to your recovery key (never inside Murage's data folder or your backup folder), then tells you where it is. Use **Save a copy of the off-site password…** to keep a second copy away from this computer: you need it, and your recovery key, to restore from the off-site copy on another computer. If you already have a password file, for example when restoring on a new computer, choose **Choose a password file I already have** instead.
