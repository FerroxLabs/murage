...
The Local VM gives each bot a containerized Linux desktop on the same machine as Murage. It is useful for browser work, downloads, and repeatable automation that should not run directly on your host desktop.
...
Local VMs always run on the same physical host as the Murage desktop process. `maxInstances: 2` does not pool one VM from a Mac and another from a Windows PC. To use Windows capacity today, run Murage on Windows with its supported Podman lane and create both per-bot desktops there. The BYO-VPS backend is limited to an x86_64 Linux Docker host; do not point it at Windows as a substitute remote Local VM. Cross-host Mac and Windows pooling is tracked in [issue #508](https://github.com/milind-soni/Murage/issues/508).
...
Local VM ownership uses a renewable lease. Murage validates the resolved image and container safety contract before reuse instead of trusting only a mutable image tag.
