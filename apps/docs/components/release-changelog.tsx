...
// Murage's installers and update feeds live in a separate public repository
// from the source, so the changelog is read from there rather than from this
// repo's tags. One source, so a release cannot appear twice.
const RELEASES_REPOSITORY = 'FerroxLabs/murage-releases';
const SOURCE_REPOSITORY = 'FerroxLabs/murage';
const RELEASES_PER_PAGE = 100;
const MAX_RELEASE_PAGES = 10;
// The body the release workflow used before it generated notes from merged
// pull requests. It says nothing a reader wants; point at the commit instead.
const PLACEHOLDER_NOTES =
  /^(?:Draft assembled by the release workflow from|Built by the release workflow from)\s+\S+@([0-9a-f]{40})\.?(?:\s+Edit these notes, then publish\.)?\s*$/i;
...
function releaseNotes(release: GitHubRelease) {
  ...
  const placeholder = PLACEHOLDER_NOTES.exec(body);
  if (!placeholder) return body;

  const commit = placeholder[1];
  return `This build predates curated release notes. [View its source commit (${commit.slice(0, 7)})](https://github.com/${SOURCE_REPOSITORY}/commit/${commit}).`;
}
...
async function fetchPublishedReleases(repository: string): Promise<GitHubRelease[]> {
  try {
    ...
    // A docs build must never fail because GitHub was briefly unavailable —
    // the page falls back to a link.
...
async function publishedReleases(): Promise<GitHubRelease[]> {
  const releases = await fetchPublishedReleases(RELEASES_REPOSITORY);
  ...
  if (releases.length === 0) {
    return (
      <p>
        ...
        <a href={`https://github.com/${RELEASES_REPOSITORY}/releases`}>
